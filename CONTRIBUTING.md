...
- A binding joins an SDK and one or more runtime overlays to either an external
  compiler installation or a managed Compiler Kit, then generates compiler
  launchers, selected target tools, an audit policy and the selected CMake,
  shell or Conan consumer integrations.
- The supplemental host build-tools artifact remains outside those four layers.
  It is native to the producer architecture, follows the Compiler Kit host
  glibc floor, and must not own compiler or target runtime inputs.
...
producer-side validation output, binding verification result, and representative
...
A build-tools change additionally requires real native x86-64 and AArch64
builds, manifest and recursive ELF validation, and execution of every published
tool before both architectures are described as qualified.
Record completed cells in the
[release qualification ledger](docs/release-qualification.md); an unlisted
catalog combination must not be described as qualified.
...
The Make workflow's default producer store is `out/store`, so reusable producer
artifacts survive `make clean`. `make purge` also removes `.venv` and all of
`out/`, including the default store. Neither command traverses an external
store or removes an installation outside the repository.
