...
- 运行时层包含编译器提供的头文件、CRT 文件以及 GCC 或 LLVM 运行时库，不包含编译器
  可执行文件。
- binding 把 SDK 和一个或多个运行时层连接到外部编译器或托管 Compiler Kit，并生成
  封装命令、检查策略和所选 CMake、shell 或 Conan 接入文件。
- 补充性的 host build-tools artifact 不属于这四层；它与 producer 原生架构一致，遵循
  Compiler Kit host glibc floor，并且不能拥有 compiler 或 target runtime 输入。
...
验证结果、binding 验证结果，以及代表性目标环境中的真实项目结果。单元测试不能认证
真实工具链矩阵；每个正式发布组合都要在声明的最低主机上运行安装器和已安装 launcher。

修改构建工具后，还必须分别在原生 x86-64 和 AArch64 环境完成真实构建，验证 manifest
和递归 ELF 闭包，并实际运行每个发布工具，之后才能声明两个架构均已通过发布认证。
完成认证的组合应写入[发布认证记录](docs/zh-CN/release-qualification.md)；未列入记录的
catalog 组合不得描述为已认证。

`make clean` 会删除仓库内的 setup 工作目录、Bundle 输出、Python 构建产物和工具缓存，
但保留 `.venv` 和默认的 `out/store`，因此可复用的生成端制品可跨普通清理继续使用。
`make purge` 还会删除 `.venv`、整个 `out/` 及其中的默认 store，但不会遍历外部 store。
两个命令都不会删除仓库外的安装目录。
