...
from linux_toolchain.compiler.managed import (
    TARGET_TOOL_NAMES as COMPILER_KIT_TARGET_TOOL_NAMES,
)
...
class ManagedBuilderTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        archive_name = f"crosstool-ng-{release.version}.tar.xz"
        release_patch = patch.dict(
            CROSSTOOL_NG_RELEASES,
            {
                release.version: replace(
                    release,
                    sha256=hashlib.sha256(archive_name.encode()).hexdigest(),
                )
            },
        )
        release_patch.start()
        ...
        *,
        ...
        manifest = {
            ...
            "build_environment": {
                ...
                "contract_sha256": identity["builder_contract"]["sha256"],
            ...
            "sources": {
                ...
                **{
                ...
                "mold": {
                    "version": "2.41.0",
                    "sha256": (
                        "0a61abac85d818437b425df856822e9d6"
                        "e9982baeae5a93bcb02fe6c0060c61a"
                    ),
                },
                "download_archives": {},
            },
    ...
    def run_paired_script_harness(
        ...
        *,
        ...
        install_experimental: bool = True,
    ) -> tuple[list[str], Path, Path]:
        target = f"{selection.target_arch}-unknown-linux-gnu"
        ...
        mold_license = roots["/sdk"] / "licenses/mold/LICENSE"
        mold_license.parent.mkdir(parents=True)
        mold_license.write_text("mold\n", encoding="utf-8")
...
""",
            ...
            (build / ".linux-toolchain-configured").write_text(
                ...
                f"backend={target}-{backend_version} "
                "projects=clang,lld linkage=both experimental=on runtime_dir=os-arch "
                "release_ldflags=-static-libstdc++ -static-libgcc "
                "-Wl,--strip-debug\n",
                encoding="utf-8",
            )
            experimental_install = (
                ': >"$DESTDIR/opt/linux-toolchain/managed/lib/libc++experimental.a"'
...
""",
    ...
    def write_compiler_backend(
        ...
        *,
        ...
        backend_patch = packaged_crosstool_ng_patch(release.version)
        archive_name = f"crosstool-ng-{release.version}.tar.xz"
        (downloads / archive_name).write_bytes(archive_name.encode())
        identity = sdk_producer_identity(spec)
        source_evidence = {
            "crosstool-ng": {
                "version": release.version,
    ...
    def workspace_manifest(
        lock,
        *,
        triplet: str = "x86_64-unknown-linux-gnu",
    ) -> dict[str, object]:
    ...
    def write_managed_licenses(
        ...
        *,
        ...
        if compiler_kit:
            required = (*required, "binutils/COPYING", "mold/LICENSE")
    ...
    def test_workspace_records_selected_sdk_and_target_tools(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            tools = self.write_target_tools(root, "x86_64-unknown-linux-gnu")
    ...
    def test_source_acquisition_reports_the_selected_compiler(self) -> None:
        compiler_id = artifact_id(
            self.lock,
            collection="compiler_kits",
            family="clang",
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                messages,
                [
                    "downloading and verifying clang 16.0.6 source",
                    "verifying cached clang 16.0.6 source",
                ],
            )

    def test_workspace_rejects_sdk_with_changed_build_inputs(self) -> None:
        runtime_id = artifact_id(self.lock, collection="runtimes", family="gcc")
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            sdk = self.write_sdk(root)
            ...
            manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
            expected_config_sha256 = manifest["defconfig_sha256"]
            ...
            manifest["defconfig_sha256"] = expected_config_sha256
            manifest["build_environment"]["contract_sha256"] = "0" * 64
            manifest_path.write_text(
                json.dumps(manifest, indent=2, sort_keys=True) + "\n",
                encoding="utf-8",
            )
            with self.assertRaisesRegex(ConfigurationError, "builder inputs changed"):
                render_workspace(
                    self.lock,
                    runtime_id,
                    root / "workspace",
    ...
    def test_host_elf_audit_rejects_the_wrong_loader(self) -> None:
        cases = (
            ("x86_64", "/lib/ld-linux-aarch64.so.1"),
            ("aarch64", "/lib64/ld-linux-x86-64.so.2"),
        )
        for arch, wrong_loader in cases:
            with self.subTest(arch=arch), tempfile.TemporaryDirectory() as directory:
                ...
                with (
                    patch(
                        "linux_toolchain.managed.artifacts.is_elf",
                        return_value=True,
                    ),
                    patch(
                        "linux_toolchain.managed.artifacts.ReadElfInspector"
                    ) as inspector,
                    self.assertRaisesRegex(ExternalToolError, "interpreter"),
                ):
                    inspector.return_value.inspect.return_value = metadata
                    _host_elf_audit(root, arch=arch, glibc_floor="2.19")
            ...
            for family in ("gcc", "clang"):
                with self.subTest(family=family):
                    ...
                    build_script = render_build_script(
                        selection,
                        triplet="x86_64-unknown-linux-gnu",
                        backend_triplet="x86_64-unknown-linux-gnu",
                        backend_version="9.5.0",
                        paired_runtime=True,
                    )
    ...
    def test_managed_libcxx_requires_the_installed_experimental_library(
        self,
    ) -> None:
        selection = select_artifact(
            self.lock,
        ...
        with (
            ...
            self.assertRaisesRegex(
                ExternalToolError,
                "managed libc\\+\\+ runtime is missing libc\\+\\+experimental\\.a",
            ),
        ):
            self.run_paired_script_harness(
                Path(directory),
                selection,
    ...
    def test_aarch64_gcc_runtime_omits_quadmath(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            log, _, runtime_output = self.run_paired_script_harness(
                Path(directory),
                compiler,
                preserve_primary=False,
                preserve_runtime=False,
            ...
            self.assertTrue(
                any(
                    line.startswith("configure ") and "--disable-libquadmath" in line
                    for line in log
                )
            )
            self.assertFalse(
            ...
            quadmath_header = (
                runtime_output
                / ".artifacts.staging/runtime/lib/gcc"
                / "aarch64-unknown-linux-gnu"
                / compiler.version
                / "include/quadmath.h"
            )
            self.assertFalse(quadmath_header.exists())
    ...
    def test_paired_gcc16_build_publishes_libatomic(self) -> None:
        ...
        compiler = select_artifact(
            ...
            next(
                ...
                if kit.family == "gcc" and kit.version == "16.1.0"
            ),
        )
        with tempfile.TemporaryDirectory() as directory:
            ...
            for preserve_primary, preserve_runtime in states:
                with (
                    ...
                    if family == "gcc":
                        ...
                        runtime_ran = any(
                            "configure-target-libgcc" in line for line in log
                        )
                        quadmath_ran = any(
    ...
    def test_aarch64_uses_a_native_arm_compiler_backend(
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            tools = self.write_target_tools(root, "aarch64-unknown-linux-gnu")
    ...
    def test_rejects_unowned_workspace_and_escaping_target_tool(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            outside.chmod(0o755)
            ar = tools / "x86_64-unknown-linux-gnu-ar"
            ar.unlink()
    ...
    def test_source_archive_hash_mismatch_removes_partial_file(self) -> None:
        source = SourceLock(
            ...
            version="13.4.0",
            kind="archive",
            url=("https://mirrors.kernel.org/gnu/gcc/gcc-13.4.0/gcc-13.4.0.tar.xz"),
            sha512="0" * 128,
        )
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(progress, [(0, 5), (5, 5)] * 3)
    ...
    def test_gcc_source_download_uses_ordered_gnu_mirrors(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch(
                ...
                side_effect=(
                    ...
                    urllib.error.URLError("first fallback unavailable"),
                    response,
                ),
            ) as urlopen:
                result = download_source_archive(source, destination)

            self.assertEqual(result.read_bytes(), payload)
            self.assertEqual(
                tuple(call.args[0].full_url for call in urlopen.call_args_list),
                (
                    "https://mirrors.kernel.org/gnu/gcc/gcc-13.4.0/gcc-13.4.0.tar.xz",
    ...
    def test_paired_gcc_build_runs_once_with_the_immutable_image_id(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn(
                "LINUX_TOOLCHAIN_JOBS=7",
                run_streaming.call_args.args[0],
            )
            docker_run = run_logged.call_args.args[0]
            self.assertIn(image_id, docker_run)
            self.assertNotIn("custom-builder:mutable", docker_run)
            self.assertIn("LINUX_TOOLCHAIN_JOBS=7", docker_run)
    ...
    def test_sdk_and_managed_builds_share_one_image_identity(self) -> None:
        platform = "linux/amd64"
        managed = managed_builder_contract(platform)
        sdk_identity = sdk_producer_identity(get_recipe("x86_64", "2.19").to_spec())
        sdk = sdk_identity["builder_contract"]
        ...
        for key in (
            ...
            self.assertEqual(managed[key], sdk[key])
    ...
    def test_compiler_finalize_publishes_loader_compatible_manifest(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                ...
                set(COMPILER_KIT_TARGET_TOOL_NAMES),
            )
            self.assertEqual(
                set(standard["locations"]["linkers"]),
                {"default", "bfd", "gold", "mold"},
            )
