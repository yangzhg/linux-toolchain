...
class BindingLinkVerificationTest(unittest.TestCase):
    ...
    def test_aarch64_runtime_verification_covers_exception_library_and_paths(
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            linker_binary.touch()
            selected_linker = compiler_bin / "aarch64-unknown-linux-gnu-ld"
            ...
            self.assertIn("#ifndef __GLIBCXX__", sources["cxx-executable.cc"])
            self.assertIn("#ifdef _LIBCPP_VERSION", sources["cxx-executable.cc"])
    ...
    def test_llvm_runtime_link_evidence_covers_shared_and_static_closure(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            runtime_environment = {
                **os.environ,
                "LINUX_TOOLCHAIN_CXX_RUNTIME": "libc++",
            }
            loader = "/lib64/ld-linux-x86-64.so.2"
            ...
            with (
                ...
                result = _verify_binding_links(
                    ...
                    cxx_link_flags=("--target=x86_64-pc-linux",),
                    environment=runtime_environment,
                    redundant_clang_stdlib="libc++",
                )
            ...
            self.assertIn("#ifndef _LIBCPP_VERSION", sources["cxx-executable.cc"])
            self.assertIn("#ifdef __GLIBCXX__", sources["cxx-executable.cc"])
            redundant_checks = [
                command
                for command in commands
                if any(
                    argument.endswith("cxx-redundant-stdlib.cc") for argument in command
                )
            ]
            self.assertEqual(len(redundant_checks), 1)
            self.assertEqual(redundant_checks[0].count("-stdlib=libc++"), 2)
            self.assertIn("-Werror=unused-command-line-argument", redundant_checks[0])
            selected_header_checks = [
                command
                for command in commands
                if any(argument.endswith("cxx-executable.cc") for argument in command)
            ]
            self.assertEqual(len(selected_header_checks), 1)
            self.assertNotIn("-stdlib=libc++", selected_header_checks[0])
            self.assertTrue(environments)
            self.assertTrue(
                all(environment == runtime_environment for environment in environments)
            )
            static_links = [
                command
                for command in commands
