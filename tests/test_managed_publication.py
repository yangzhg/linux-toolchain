...
from linux_toolchain.managed.publication import (
    ...
    load_managed_llvm_source_evidence,
...
class ManagedPublicationTest(unittest.TestCase):
    ...
    def test_loads_a_runtime_that_matches_its_lock(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(loaded.target, "x86_64-unknown-linux-gnu")
    ...
    def test_publishes_gcc_with_locked_metadata(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                patch(
                    "linux_toolchain.runtime.importer.materialize_gcc_runtime",
    ...
    def test_runtime_set_reuses_complete_components_and_protects_ownership(
        self,
    ) -> None:
        variant = next(item for item in self.lock.variants if item.family == "clang")
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            publications = {}
            source_files = {}
            for runtime in variant.runtimes:
                component = root / "components" / runtime.runtime_id
                component.mkdir(parents=True)
                payload = component / "payload.txt"
                payload.write_text(runtime.kind, encoding="utf-8")
                source_files[runtime.kind] = payload
                publications[runtime.runtime_id] = SimpleNamespace(
                    root=component,
                    selection=SimpleNamespace(artifact_id=runtime.runtime_id),
                )

            def load_component(
                _lock, artifact_id: str, publication: Path
            ) -> SimpleNamespace:
                return SimpleNamespace(
                    root=Path(publication).resolve(),
                    selection=SimpleNamespace(artifact_id=artifact_id),
                )

            output = root / "runtime-set"
            with patch(
                "linux_toolchain.managed.publication.load_managed_runtime_publication",
                side_effect=load_component,
            ):
                published = publish_managed_runtime_set(
                    self.lock,
                    variant.id,
                    publications,
                    output,
                )
                reused = publish_managed_runtime_set(
                    self.lock,
                    variant.id,
                    publications,
                    output,
            ...
            self.assertEqual(published.root, output.resolve())
            self.assertEqual(reused.root, published.root)
            self.assertEqual(
                [entry.kind for entry in published.entries],
                ["libstdc++", "libc++"],
            )
            manifest = json.loads(
                (output / "manifest.json").read_text(encoding="utf-8")
            )
            self.assertEqual(manifest["default"], "libstdc++")
            self.assertEqual(
                [entry["path"] for entry in manifest["runtimes"]],
                ["runtimes/libstdcxx", "runtimes/libcxx"],
            )
            for kind, directory in (
                ("libstdc++", "libstdcxx"),
                ("libc++", "libcxx"),
            ):
                destination = output / "runtimes" / directory / "payload.txt"
                self.assertEqual(
            ...
            self.assertEqual(
                (unowned / "keep").read_text(encoding="utf-8"),
                "user data\n",
            )
