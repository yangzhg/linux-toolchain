...
class CompilerKitTest(unittest.TestCase):
    ...
    def create_kit(
        ...
        *,
        ...
        target = f"{target_arch}-unknown-linux-gnu"
        ...
        linkers = {
            "default": self.executable(payload / "bin" / f"{target}-ld"),
            "bfd": self.executable(payload / "bin" / f"{target}-ld.bfd"),
            "gold": self.executable(payload / "bin" / f"{target}-ld.gold"),
            "mold": self.executable(payload / "bin" / f"{target}-ld.mold"),
        }
        if provider == "clang":
            linkers["lld"] = self.executable(payload / "bin" / "ld.lld")
    ...
    def test_loads_gcc_and_clang_kit_entrypoints(self) -> None:
        for provider, version in (("gcc", "10.5.0"), ("clang", "22.1.0")):
            with (
                ...
                self.assertEqual(
                    tuple(loaded.linkers),
                    (
                        ("bfd", "default", "gold", "lld", "mold")
                        if provider == "clang"
                        else ("bfd", "default", "gold", "mold")
                    ),
                )
                self.assertIsInstance(loaded.linkers, MappingProxyType)
    ...
    def test_rejects_invalid_target_architecture_and_triplet(self) -> None:
        cases = (
            ...
            (
                "incompatible",
                {"arch": "aarch64", "triplet": "x86_64-unknown-linux-gnu"},
            ),
            (
                "incompatible",
