...
def write_build_outputs(root: Path, spec: SdkSpec, *, full: bool) -> None:
    ...
    for path in executables:
        ...
        path.chmod(0o755)
    mold = toolchain / spec.target.triplet / "bin" / "mold"
    mold.parent.mkdir(parents=True, exist_ok=True)
    mold.write_bytes(b"#!/bin/sh\n")
    mold.chmod(0o755)
    mold_link = mold.parent / "ld.mold"
    if not mold_link.exists():
        mold_link.symlink_to("mold")
...
class CrosstoolNgConfigTest(unittest.TestCase):
    def test_sdk_identity_uses_content_and_builder_contract_not_mirror_url(
        ...
        with patch.dict(os.environ, {UBUNTU_BUILDER_SNAPSHOT_ENV: ""}):
            ...
            with patch.dict(
                ...
                self.assertNotEqual(sdk_producer_identity(spec), baseline)
            with patch(
                "linux_toolchain.sdk.crosstool_ng.packaged_builder_dockerfile_sha256",
                return_value="1" * 64,
            ):
                self.assertNotEqual(sdk_producer_identity(spec), baseline)
            backend_patch = packaged_crosstool_ng_patch(spec.builder.version)
            with patch(
                "linux_toolchain.sdk.crosstool_ng.packaged_crosstool_ng_patch",
                return_value=PackagedPatch(
                    relative_path=backend_patch.relative_path,
                    content=backend_patch.content + b"\n",
                ),
            ):
                self.assertNotEqual(sdk_producer_identity(spec), baseline)
        with patch.dict(
    ...
    def test_explicit_source_archives_share_a_content_addressed_cache(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            cache = root / "store/source-archives"
            ...
            self.assertFalse(tuple(cache.rglob("*.part-*")))

    def test_source_archives_use_ordered_https_mirrors(self) -> None:
        with patch.dict(
            os.environ,
            {"LINUX_TOOLCHAIN_GNU_MIRROR": ""},
        ):
            archives = {
                archive.filename: archive
                for archive in _component_archives(glibc_spec()).values()
            }
            sources = {
                archive.filename: archive
                for archive in _source_archives(glibc_spec()).values()
            }
        ...
        self.assertEqual(
            archives["gcc-9.5.0.tar.xz"].source_urls,
            (
                "https://mirrors.kernel.org/gnu/gcc/gcc-9.5.0/gcc-9.5.0.tar.xz",
                "https://ftpmirror.gnu.org/gcc/gcc-9.5.0/gcc-9.5.0.tar.xz",
        ...
        self.assertTrue(
            archives["binutils-2.43.1.tar.xz"].source_url.startswith("https://")
        )
        self.assertEqual(
            sources["gmp-6.3.0.tar.xz"].source_urls,
            (
        ...
        self.assertEqual(
            sources["v2.41.0.tar.gz"].source_url,
            "https://github.com/rui314/mold/archive/refs/tags/v2.41.0.tar.gz",
        )
        self.assertEqual(
            sources["v2.41.0.tar.gz"].sha256,
    ...
    def test_renders_exact_glibc_floor_and_complete_backend(self) -> None:
        ...
        self.assertIn("CT_BINUTILS_LINKER_LD_GOLD=y", config)
        self.assertIn("CT_LINKER_MOLD=y", config)
        self.assertIn("CT_MOLD_V_2_41_0=y", config)
    ...
    def test_crosstool_ng_1_28_family_renders_exact_component_keys(self) -> None:
        ...
        self.assertIn("CT_BINUTILS_V_2_43=y", config)
    ...
    def test_rejects_new_binutils_with_old_aarch64_glibc(self) -> None:
        incompatible = replace(
            aarch64_glibc_spec(),
            builder=replace(aarch64_glibc_spec().builder, binutils="2.43.1"),
        )
    ...
    def test_build_runs_host_preflight_before_downloads(self) -> None:
        packaged = packaged_builder_dockerfile()
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            spec = glibc_spec()
            render_workspace(spec, root)
    ...
    def test_build_reuses_only_a_matching_complete_toolchain(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                ...
                build()
                self.assertIn(
                    "sdk: acquiring linux linux-6.12.41.tar.xz",
                    progress_messages,
                )
                self.assertIn(
                    "sdk: acquiring mold v2.41.0.tar.gz",
                    progress_messages,
                )
                self.assertIn(
                    "sdk: acquiring crosstool-NG crosstool-ng-1.28.0.tar.xz",
                    progress_messages,
                )
                self.assertTrue(download_archive.call_args_list)
                self.assertTrue(
                    all(
                        call.kwargs["progress"] is source_progress
                        for call in download_archive.call_args_list
                    )
                )
                self.assertIs(
                    backend_archive.call_args.kwargs["source_progress"],
                    source_progress,
                )
                ...
                self.assertEqual(
                    downloaded["v2.41.0.tar.gz"].sha256,
                    "0a61abac85d818437b425df856822e9d6e9982baeae5a93bcb02fe6c0060c61a",
                )
                self.assertEqual(
                    run_build.call_args.args[0][-3:],
                    ["ct-ng", "build.7", "STOP=libc_main"],
                )
                ...
                backend_patch = packaged_crosstool_ng_patch("1.28.0")
                changed_patch = PackagedPatch(
                    relative_path=backend_patch.relative_path,
                    content=backend_patch.content + b"\n",
                )
                ...
                self.assertEqual(
                    workspace["builder_patch"]["sha256"],
                    changed_patch.sha256,
                )
...
class WorkspaceProvenanceTest(unittest.TestCase):
        ...
        evidence.update(
            {
                ...
                for index, archive in enumerate(_source_archives(spec).values())
            }
        )
        return evidence
    ...
    def record_builder(
        self,
        root: Path,
        image_name: str = "linux-toolchain-builder:1.28.0",
        *,
        dockerfile_sha256: str | None = None,
    ) -> dict[str, object]:
        spec = load_workspace(root)
        contract = _builder_contract(
            spec,
            packaged_builder_dockerfile_sha256(),
        )
        if dockerfile_sha256 is not None:
            contract = replace(
                contract,
        ...
        image = {
            "Id": "sha256:" + "a" * 64,
            "RepoDigests": ["linux-toolchain-builder@sha256:" + "b" * 64],
            "Os": "linux",
            "Architecture": "amd64",
        }
        _record_builder_provenance(
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                manifest["build_environment"],
                {
                    ...
                    "contract_sha256": sdk_producer_identity(spec)["builder_contract"][
                        "sha256"
                    ],
                    ...
                    "image": {
                        "name": "linux-toolchain-builder:1.28.0",
                        "id": image["Id"],
            ...
            backend_patch = packaged_crosstool_ng_patch("1.28.0")
            self.assertEqual(
                manifest["sources"]["crosstool-ng"]["patch"],
                {
                    "path": backend_patch.relative_path,
                    "sha256": backend_patch.sha256,
                },
            )
            self.assertEqual(manifest["sources"]["mold"]["version"], "2.41.0")
            self.assertEqual(
                manifest["sources"]["mold"]["sha256"],
                "0a61abac85d818437b425df856822e9d6e9982baeae5a93bcb02fe6c0060c61a",
            )
            ...
            self.assertIn("licenses/mold/LICENSE", manifest["licenses"]["files"])
