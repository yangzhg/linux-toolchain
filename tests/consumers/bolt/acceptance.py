...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default="Release",
        help="consumer build configuration passed to binding verification",
    )
    parser.add_argument(
        "--runner",
        help="optional target runner passed to binding verification",
    )
    parser.add_argument(
        "--skip-verify",
        action="store_true",
        help="skip generic binding verification",
    )
    parser.add_argument(
        "--skip-build",
        action="store_true",
...
def _commands(
    ...
    if not args.dry_run:
        ...
        if not args.skip_verify:
            ...
            _require_file(
                verification_entry,
                f"{args.verify_integration} integration entry point",
            )
        if not args.skip_build:
            _require_file(
    ...
    if not args.skip_verify:
        required_integrations.add(args.verify_integration)
    if not args.skip_build:
    ...
    if not args.skip_verify:
        ...
        if args.runner:
            verification.extend(("--runner", args.runner))
        commands.append(("binding verification", verification))
    if not args.skip_build:
        commands.append(
            (
                "Bolt build",
