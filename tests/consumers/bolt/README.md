...
the caller-provided checkout through Bolt's public Makefile, verifies the
selected binding integration, and audits the selected Bolt outputs.
...
2. validates the binding with the generic CMake verification project;
...
Use `--skip-build` to audit a completed build, `--skip-verify` to omit binding
verification, or `--verify-integration shell|conan` to exercise another binding
adapter. `--build-type` configures the verification project; `--make-target`
