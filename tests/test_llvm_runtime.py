...
from linux_toolchain.runtime.llvm_models import (
    LLVM_EXPERIMENTAL_STATIC_LIBRARY,
...
class LlvmRuntimeTest(unittest.TestCase):
    ...
    @classmethod
    def make_prefix(cls, root: Path, *, experimental: bool = True) -> Path:
        ...
        if experimental:
            (library / LLVM_EXPERIMENTAL_STATIC_LIBRARY).write_bytes(b"!<arch>\n")
        for relative in (
            "llvm/LICENSE.TXT",
            "clang/LICENSE.TXT",
    ...
    def import_fixture(
        ...
        *,
        ...
        prefix = self.make_prefix(root, experimental=experimental)
    ...
    def test_managed_llvm_runtime_requires_the_experimental_archive(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            _, manifest = self.import_fixture(Path(directory))
        data = manifest.to_dict()
        locations = dict(data["locations"])
        locations["static_libraries"] = [
            path
            for path in locations["static_libraries"]
            if Path(path).name != LLVM_EXPERIMENTAL_STATIC_LIBRARY
        ]
        ...
        with self.assertRaisesRegex(
            ...
            LlvmRuntimeManifest.from_dict(data)

        locations["static_libraries"] = sorted(
            [
                *locations["static_libraries"],
                f"runtime/lib/{LLVM_EXPERIMENTAL_STATIC_LIBRARY}",
            ]
        )
        data["locations"] = locations
        loaded = LlvmRuntimeManifest.from_dict(data)
        self.assertEqual(loaded.provider["major"], 22)
    ...
    def test_accepts_exact_external_clang_probe_without_recording_paths(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            prefix = self.make_prefix(root, experimental=False)
            (prefix / "lib/libc++experimental.so.1").write_bytes(b"not selected\n")
            resource = prefix / "lib/clang/22"
            ...
            self.assertNotIn(
                "libc++experimental.so.1",
                {Path(path).name for path in manifest.locations["shared_libraries"]},
            )
