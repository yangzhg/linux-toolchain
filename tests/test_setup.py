...
from linux_toolchain.managed import ManagedLock, resolve_lock, write_lockfile
...
def validated_artifacts() -> tuple[
    ...
    return (
        cast(BuildToolsArtifact, SimpleNamespace(root=Path("/tmp/build-tools"))),
        cast(ManagedCompilerArtifact, object()),
        cast(ManagedRuntimeSetPublication, object()),
    )
...
def create_prepared(
    ...
    *,
    ...
    runtime = artifact_paths.runtime_set
    build_tools = build_tools_workspace / "artifact"
    build_tools.mkdir(parents=True)
    compiler_kit.mkdir(parents=True)
    ...
    prepared = PreparedSetup(
        ...
        build_tools_workspace=build_tools_workspace,
...
class SetupConfigTest(unittest.TestCase):
    def test_setup_separates_work_directory_from_installation(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            prepared = PreparedSetup(
                ...
                build_tools=store.root / "build-tools/artifact",
                integration="conan",
                smoke_result=state / "smoke-conan/result.json",
                conan=None,
            )
            build_tools_artifact, compiler_artifact, runtime_set_publication = (
                validated_artifacts()
            )
            prepared_inputs = LeasedSetupInputs(
                lock=lock,
            ...
            def publish(*args: object, **kwargs: object) -> Path:
            ...
            with (
                ...
                patch(
                    "linux_toolchain.setup.publish_installation_from_validated_inputs",
                    side_effect=publish,
                ) as publisher,
            ):
    ...
    def test_clang_always_has_both_runtimes_and_selects_libstdcxx_version(self) -> None:
        config = SetupConfig.from_dict(
            setup_config(compiler="clang@22", libstdcxx="gcc@10")
        )
        lock = resolve_lock(config.managed_spec())

        self.assertEqual(lock.variants[0].version, "22.1.8")
        self.assertEqual(lock.variants[0].cxx_runtimes, ("libstdc++", "libc++"))
        self.assertEqual(
            {
                (runtime.provider_family, runtime.provider_version)
                for runtime in lock.runtimes
            },
            {("gcc", "10.5.0"), ("llvm", "22.1.8")},
        )

        default = SetupConfig.from_dict(setup_config(compiler="clang@22"))
        self.assertEqual(default.libstdcxx, "gcc@12")

    def test_schema_unknown_fields_and_libstdcxx_mismatches_fail_closed(self) -> None:
        ...
        with self.assertRaisesRegex(ConfigurationError, "must be omitted"):
            SetupConfig.from_dict(setup_config(libstdcxx="gcc@10"))

        with self.assertRaisesRegex(ConfigurationError, "must use gcc@VERSION"):
            SetupConfig.from_dict(setup_config(compiler="clang@22", libstdcxx="libc++"))

        custom_conan = SetupConfig.from_dict(
            setup_config(
                integration="shell",
                conan={"cppstd": "gnu17", "build_type": "Debug"},
            )
        )
        self.assertEqual(custom_conan.smoke_integration, "shell")
        self.assertEqual(custom_conan.conan_settings().cppstd, "gnu17")
        self.assertEqual(custom_conan.conan_settings().build_type, "Debug")
        self.assertEqual(
            SetupConfig.from_dict(custom_conan.to_dict()),
            custom_conan,
        )
        serialized_conan = custom_conan.to_dict()["conan"]
        self.assertIsInstance(serialized_conan, dict)
        self.assertNotIn("build_profile", serialized_conan)

        with self.assertRaisesRegex(ConfigurationError, "Conan verification"):
            SetupConfig.from_dict(
                setup_config(
                    integration="shell",
                    conan={"build_profile": "native"},
                )
            )
...
class SetupWorkflowTest(unittest.TestCase):
    def setUp(self) -> None:
        def build_tools(
            ...
            **kwargs: object,
            ...
            return cast(BuildToolsArtifact, SimpleNamespace(root=artifact.resolve()))

        builder = patch(
            "linux_toolchain.setup.build_build_tools",
    ...
    def test_force_repairs_only_a_matching_invalid_sdk_workspace(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIs(
                builder.call_args.kwargs["source_progress"],
                source_progress,
            )
    ...
    def test_shared_ready_store_skips_managed_diagnostics_and_sdk_build(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            write_json(
                ...
                setup_config(
                    integration="shell",
                    conan={"cppstd": "gnu20", "build_type": "Debug"},
                ),
            )
            config = SetupConfig.load(config_path)
            target_sdk = sdk_spec(config)
            ...
            def smoke(args: object) -> int:
                smoke_build_types.append(args.build_type)
                write_smoke_fixture(
            ...
            with (
                ...
                patch("linux_toolchain.setup._prepare_conan") as conan_preparer,
            ...
            self.assertEqual(diagnostics_seen, ["managed"])
            sdk_builder.assert_not_called()
            conan_preparer.assert_not_called()
            conan_settings = assembler.call_args.kwargs["conan"]
            self.assertEqual(conan_settings.cppstd, "gnu20")
            self.assertEqual(conan_settings.build_type, "Debug")
            self.assertEqual(smoke_build_types, ["Release"])
    ...
    def test_prepare_orchestrates_one_variant_and_writes_machine_local_state(
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            artifact_paths.runtime_set.mkdir(parents=True)
            with (
            ...
            self.assertEqual(prepared.runtime, artifact_paths.runtime_set)
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            def ensure_sdk(
                ...
                **kwargs: object,
                ...
                self.assertIs(kwargs["source_progress"], source_progress)
            ...
            def assemble(
                ...
                **kwargs: object,
                ...
                current_config = SetupConfig.from_dict(selection)
                artifact_paths = variant_artifact_paths(
                    _lock,
                    variant_id,
                    managed_workspace,
                    sdk_spec(current_config),
                    compiler_backend_spec(current_config),
                )
                compiler_kit = artifact_paths.compiler_kit
                runtime = artifact_paths.runtime_set
                compiler_kit.mkdir(parents=True, exist_ok=True)
            ...
            self.assertEqual(
                sdk_calls,
                [
                    (expected_sdk, SDK_BUILD_GOAL, store.source_archive_cache),
                    (expected_backend, FULL_BUILD_GOAL, store.source_archive_cache),
                ]
                * 2,
            )
            self.assertEqual(
    ...
    def test_prepare_reuses_matching_prepared_state_without_diagnostics(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                ...
                patch("linux_toolchain.setup.run_diagnostics") as diagnostics,
            ):
            ...
            self.assertEqual(
                progress_messages,
                ["setup: verification PASS (conan, cached)"],
            )

    def test_prepare_refreshes_changed_producer_inputs_without_force(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            config_path, state, _, prepared = create_prepared(
                root,
            ...
            with (
                patch(
                    "linux_toolchain.managed.identity._RUNTIME_ADAPTER_REVISION",
                    2,
                ),
                patch(
                    "linux_toolchain.setup._diagnose",
                    side_effect=ConfigurationError("refresh reached"),
                ) as diagnostics,
                self.assertRaisesRegex(ConfigurationError, "refresh reached"),
            ):
                prepare_setup(config_path, state_directory=state)

            diagnostics.assert_called_once()
            refreshed = PreparedSetup.load(state / "prepared.json")
            self.assertEqual(refreshed.runtime, prepared.runtime)
            self.assertIsNone(refreshed.smoke_result)
