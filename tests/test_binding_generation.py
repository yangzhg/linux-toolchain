...
class BindingGenerationTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        resolver = patch(
            "linux_toolchain.compiler.binding.resolve_compiler_target_tools",
            return_value=self.target_tools,
        )
        resolver.start()
        self.addCleanup(resolver.stop)
        identity = patch(
            "linux_toolchain.compiler.binding.capture_executable_identity",
        ...
        for helper in (
            ...
            "_verify_linker_choices",
        ):
            helper_patch = patch(
                f"linux_toolchain.compiler.binding.{helper}",
                return_value={"status": "passed", "choices": []},
            )
            helper_patch.start()
            self.addCleanup(helper_patch.stop)
        linker = patch(
            "linux_toolchain.compiler.binding.resolve_compiler_tool",
    ...
    def runtime_entry(
        ...
        runtimes = manifest["cxx_runtimes"]
        ...
        if len(matches) != 1:
            raise AssertionError(f"missing runtime manifest entry: {kind}")
        return matches[0]

    @staticmethod
    def write_recording_clang(
        path: Path,
        *,
        version: str,
        default_target: str,
...
for argument in "$@"; do
  case "$argument" in
    --version) printf '%s\\n' 'clang version {version}'; exit ;;
    -dumpmachine) printf '%s\\n' '{default_target}'; exit ;;
    -Wl,-rpath-link,*) exit 97 ;;
  esac
done
printf '%s\\n' "$@"
""",
    ...
    def create_compiler_kit(
        ...
        *,
        ...
        driver_body = f"""case " $* " in
  *" -dumpmachine "*) printf '%s\\n' '{reported_target}' ;;
  *) printf '%s\\n' '{version_text}' ;;
esac"""
        cc = executable(cc_name, driver_body)
        cxx = executable(cxx_name, driver_body)
        tools = {name: executable(f"{target}-{name}") for name in TARGET_TOOL_NAMES}
        linkers = {
            "default": executable(f"{target}-ld"),
            "bfd": executable(f"{target}-ld.bfd"),
            "gold": executable(f"{target}-ld.gold"),
            "mold": executable(f"{target}-ld.mold"),
        }
        if provider == "clang":
            linkers["lld"] = executable("ld.lld")
        ...
        manifest: dict[str, object] = {
            ...
            "locations": {
                ...
                "linkers": {
                    name: path.relative_to(kit).as_posix()
                    for name, path in linkers.items()
                },
            },
        }
    ...
    def create_managed_binding_fixture(self, root: Path) -> SimpleNamespace:
        ...
        runtime_id = variant.runtime_id("libstdc++")
        runtime_selection = select_artifact(lock, runtime_id)
        ...
        runtime_set_root = root / "runtime-set"
        runtime_set_root.mkdir()
        runtime_set_publication = ManagedRuntimeSetPublication(
            root=runtime_set_root.resolve(),
            manifest_path=(runtime_set_root / "manifest.json").resolve(),
            variant=variant,
            entries=(
                ManagedRuntimeSetEntry(
                    kind="libstdc++",
        ...
        with (
            ...
            patch(
                "linux_toolchain.compiler.managed_binding.load_managed_runtime_set_publication",
                return_value=runtime_set_publication,
            ),
            patch(
    ...
    def create_llvm_runtime(
        ...
        *,
        ...
        for crt_object in crt_objects:
            crt_object.touch()
        libraries = tuple(
            library_dir / f"{name}.so" for name in ("libc++", "libc++abi", "libunwind")
        )
    ...
    def test_generates_selected_integrations_and_runtime_manifest(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn(
                "export LINUX_TOOLCHAIN_TARGET='x86_64-unknown-linux-gnu'",
                environment,
            )
                    ...
                    "available": [
                        {
                            ...
                            "note": (
                                "C++ runtime symbol requirements are audited but "
                                "are not bounded by this glibc-floor binding."
                            ),
                        }
                    ],
                },
    ...
    def test_managed_binding_uses_only_manifest_selected_tools(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            linker_locations = locations["linkers"]
            assert isinstance(linker_locations, dict)
            self.assertEqual(
                manifest["compiler"]["tools"]["ld"]["invocation_path"],
                str((kit / str(linker_locations["default"])).resolve()),
            )
            self.assertEqual(
                self.link_verifier.call_args.kwargs["linker_executable"],
                (kit / str(linker_locations["default"])).resolve(),
            )

    def test_gcc_before_12_publishes_a_fixed_mold_selector_driver(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            sdk = self.create_sdk(root)
            kit, _ = self.create_compiler_kit(root, version="10.4.0")
            runtime, runtime_manifest = self.create_runtime(
                root,
                target="x86_64-unknown-linux-gnu",
                major=10,
            ...
            cc_mold = (output / "bin/cc-mold").read_text(encoding="utf-8")
            self.assertIn(f"-B{output / 'libexec/mold'}/", cc_mold)
            self.assertEqual(
                (output / "libexec/mold/ld").resolve(),
                (output / "libexec/ld.mold").resolve(),
            )
            self.assertEqual(
                (output / "libexec/mold/x86_64-unknown-linux-gnu-ld").resolve(),
                (output / "libexec/ld.mold").resolve(),
            )
            manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
            self.assertIn("cc-mold", manifest["compiler"]["aliases"])
            self.assertIn("c++-mold", manifest["compiler"]["aliases"])
    ...
    def test_managed_binding_rejects_driver_version_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            runtime, _ = self.create_runtime(root, target="x86_64-unknown-linux-gnu")
    ...
    def test_managed_binding_rejects_driver_default_target_mismatch(self) -> None:
        for provider, version in (("gcc", "13.4.0"), ("clang", "22.1.8")):
            with (
                ...
                kit, _ = self.create_compiler_kit(
                    ...
                    provider=provider,
                    version=version,
                    reported_target="aarch64-unknown-linux-gnu",
                )

                with self.assertRaisesRegex(
                    ConfigurationError,
                    "C driver target mismatch",
                ):
                    managed_compiler_info(load_compiler_kit(kit))
    ...
    def test_managed_clang_can_use_a_pinned_gcc_runtime(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            pinned = self.runtime_entry(manifest, "libstdc++")
            self.assertEqual(pinned["provider"]["version"], "13.4.0")
    ...
    def test_managed_clang_pins_shared_llvm_runtime_and_selectors(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            for expected in ("-resource-dir", str(resource_dir)):
                self.assertIn(expected, c_wrapper)
                self.assertIn(expected, cxx_wrapper)
            for expected in ("--rtlib=compiler-rt", "--unwindlib=libunwind"):
                self.assertIn(expected, c_wrapper)
                self.assertIn(expected, cxx_wrapper)
            self.assertNotIn("--target=", c_wrapper)
            self.assertNotIn("--target=", cxx_wrapper)
            self.assertNotIn("-stdlib=", c_wrapper)
            self.assertIn("-stdlib=libc++", cxx_wrapper)
            self.assertNotIn("-nostdinc++", cxx_wrapper)
            libcxx_headers = output / "include/c++/v1"
            self.assertTrue(libcxx_headers.is_symlink())
            self.assertEqual(libcxx_headers.resolve(), cxx_include)

            compatible_target = "--target=x86_64-pc-linux"
            config = root / "clang.cfg"
            config.write_text(f"{compatible_target}\n", encoding="utf-8")
            ...
            self.assertIn(f"--config={config}", arguments)

            static_unwind = next(
                path
                for path in runtime_manifest.locations["static_libraries"]
                if Path(path).name == "libunwind.a"
            )
            shared_unwind = next(
                path
                for path in runtime_manifest.locations["shared_libraries"]
                if Path(path).name == "libunwind.so"
            )
            static_entry = (output / "runtime-link/libunwind.a").read_text(
                encoding="utf-8"
            )
            shared_entry = (output / "runtime-link/libunwind.so").read_text(
                encoding="utf-8"
            )
            self.assertIn(str((runtime / static_unwind).resolve()), static_entry)
            self.assertIn("-lpthread", static_entry)
            self.assertIn("-ldl", static_entry)
            self.assertIn(str((runtime / shared_unwind).resolve()), shared_entry)

            linker_wrapper = (output / "libexec/ld").read_text(encoding="utf-8")
            self.assertIn(str(output / "bin/ld"), linker_wrapper)
            self.assertIn("-rpath-link", linker_wrapper)
            self.assertIn(
                str(
                    (runtime / runtime_manifest.locations["library_dirs"][0]).resolve()
                ),
                linker_wrapper,
            )
            ...
            manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
            pinned = self.runtime_entry(manifest, "libc++")
            self.assertEqual(manifest["cxx_runtimes"]["default"], "libc++")
    ...
    def test_managed_clang_exposes_both_native_runtime_choices(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            sdk = self.create_sdk(root)
            kit, kit_manifest = self.create_compiler_kit(
            ...
            gcc_runtime, gcc_manifest = self.create_runtime(
                root,
                target="x86_64-unknown-linux-gnu",
            )
            llvm_runtime, llvm_manifest = self.create_llvm_runtime(root)
            with (
                patch(
                    "linux_toolchain.compiler.runtime_binding.validate_runtime_manifest",
                    return_value=gcc_manifest,
                ),
                patch(
                    "linux_toolchain.compiler.runtime_binding.validate_llvm_runtime_manifest",
                    return_value=llvm_manifest,
                ),
            ):
                runtimes = RuntimeBindingSet(
                    default_kind="libstdc++",
                    bindings=(
                        ("libstdc++", load_runtime_binding(gcc_runtime)),
                        ("libc++", load_runtime_binding(llvm_runtime)),
                    ),
                )
                loaded_kit = load_compiler_kit(kit)
                manifest_path = create_binding_from_inputs(
                    sdk,
                    root / "binding",
                    managed_compiler_info(loaded_kit),
                    runtime=runtimes,
                    toolchain=managed_compiler_toolchain(loaded_kit),
                    integrations=("cmake", "shell", "conan"),
                    conan=ConanSettings(),
                )
            ...
            wrapper = (output / "bin/c++").read_text(encoding="utf-8")
            gcc_dir = (
                gcc_runtime / gcc_manifest.locations["gcc_runtime_dir"]
            ).resolve()
            resource_dir = (
                llvm_runtime / llvm_manifest.locations["resource_dir"]
            ).resolve()
            self.assertIn(f"'--gcc-install-dir={gcc_dir}'", wrapper)
            self.assertIn(f"'-resource-dir' '{resource_dir}'", wrapper)
            self.assertIn(f"'-ccc-install-dir' '{output / 'bin'}'", wrapper)
            self.assertNotIn("--target=", wrapper)
            self.assertIn('case "${LINUX_TOOLCHAIN_CXX_RUNTIME-}"', wrapper)
            self.assertNotIn("-nostdinc++", wrapper)
            self.assertEqual(
                (output / "include/c++/v1").resolve(),
                (
                    llvm_runtime / llvm_manifest.locations["cxx_include_dirs"][0]
                ).resolve(),
            )
            ...
            self.assertEqual(arguments.count("--target=x86_64-pc-linux"), 1)
            self.assertEqual(arguments.count("-stdlib=libc++"), 2)

            selected_arguments = subprocess.run(
                [
                    output / "bin/c++",
                    "-mfma",
                    "-mavx2",
                ...
                env={**os.environ, "LINUX_TOOLCHAIN_CXX_RUNTIME": "libc++"},
                check=True,
                text=True,
                capture_output=True,
            ).stdout.splitlines()
            self.assertIn("-mfma", selected_arguments)
            self.assertIn("-mavx2", selected_arguments)
            for flag in (
                "-stdlib=libc++",
                "--rtlib=compiler-rt",
                "--unwindlib=libunwind",
            ):
                self.assertEqual(selected_arguments.count(flag), 1)

            manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
            self.assertEqual(manifest["cxx_runtimes"]["default"], "libstdc++")
            self.assertEqual(
                [entry["kind"] for entry in manifest["cxx_runtimes"]["available"]],
                ["libstdc++", "libc++"],
            )
            for command in ("ld", "ld.bfd", "ld.gold", "ld.mold", "ld.lld"):
                self.assertTrue((output / "bin" / command).is_symlink())
                self.assertTrue((output / "libexec" / command).is_file())
    ...
    def test_llvm_runtime_requires_matching_managed_clang_and_libcxx(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                ...
                self.assertRaisesRegex(
                    ConfigurationError, "Conan libstdc\\+\\+ requires"
                ),
            ):
                self.create_fixture_managed_binding(
                    sdk,
...
"""
            ...
            self.assertNotIn("--target=", cc_wrapper)
            self.assertNotIn("--target=", cxx_wrapper)
            ...
            self.assertIn(f"-B{output / 'libexec'}/", cxx_wrapper)

            response = root / "compile.rsp"
            response.write_text("-c\n-Werror\nprobe.cc\n", encoding="utf-8")
            for invocation in (
                (f"@{response}",),
                ("-Werror", "-x", "c++-header", "probe.hpp", "-o", "probe.gch"),
            ):
                result = subprocess.run(
                    [output / "bin/c++", *invocation],
                    check=False,
                    text=True,
                    capture_output=True,
                )
    ...
    def test_clang_binding_reuses_imported_gcc_runtime_without_gcc_frontend(
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertNotIn(str(cxx_include), cc_wrapper)
            self.assertNotIn("-nostdinc++", cxx_wrapper)
            self.assertNotIn(str(cxx_include), cxx_wrapper)
            self.assertNotIn("-stdlib=", cxx_wrapper)
            self.assertNotIn("--rtlib=", cxx_wrapper)
            self.assertNotIn("--unwindlib=", cxx_wrapper)
            ...
            self.assertNotIn(f"--ld-path={output / 'libexec/ld'}", link_arguments)
            self.assertEqual(
                [
                    argument
    ...
    def test_generates_aarch64_binding_and_conan_armv8_profile(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            wrapper = (output / "bin" / "c++").read_text(encoding="utf-8")
            self.assertNotIn("--target=aarch64-unknown-linux-gnu", wrapper)
            ...
            manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
            self.assertEqual(manifest["sdk"]["triplet"], "aarch64-unknown-linux-gnu")
