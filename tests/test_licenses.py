...
class LicenseEvidenceTest(unittest.TestCase):
    def test_extracts_required_mold_license(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            archive = root / "mold.tar"
            write_archive(
                archive,
                {
                    "LICENSE": b"MIT\n",
                    "third-party/zstd/LICENSE": b"BSD\n",
                },
            )
            with tarfile.open(archive, mode="a") as source:
                duplicate = tarfile.TarInfo("source-1.0/third-party/zstd/LICENSE_A2")
                duplicate.type = tarfile.SYMTYPE
                duplicate.linkname = "LICENSE"
                source.addfile(duplicate)
            artifact = root / "artifact"
            artifact.mkdir()

            extract_component_licenses(archive, artifact, "mold")

            self.assertEqual(
                license_evidence(artifact, context="test artifact")["files"],
                [
                    "licenses/mold/LICENSE",
                    "licenses/mold/third-party/zstd/LICENSE",
                ],
            )
