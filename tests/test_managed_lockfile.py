...
class ManagedLockfileTest(unittest.TestCase):
    ...
    def test_resolution_builds_a_deduplicated_artifact_dag(self) -> None:
        ...
        self.assertEqual(len(lock.variants), 8)
        self.assertEqual(
    ...
    def test_clang_variants_resolve_both_exact_runtime_sources(self) -> None:
        lock = resolve_lock(self.spec)
        clang_variants = [
        ...
        for variant in clang_variants:
            selected = {
                runtime.kind: next(
                    item for item in lock.runtimes if item.id == runtime.runtime_id
                )
                for runtime in variant.runtimes
            }
            self.assertEqual(variant.default_cxx_runtime, "libstdc++")
            self.assertEqual(variant.cxx_runtimes, ("libstdc++", "libc++"))
            self.assertEqual(
                (
                    selected["libstdc++"].provider_family,
                    selected["libstdc++"].provider_version,
                ),
                ("gcc", "13.4.0"),
            )
            self.assertEqual(
                (
                    selected["libc++"].provider_family,
                    selected["libc++"].provider_version,
                ),
                ("llvm", variant.version),
            )
    ...
    def test_gcc_runtime_follows_each_exact_compiler_version(self) -> None:
        ...
        for variant in lock.variants:
            ...
            runtime = next(
                item
                for item in lock.runtimes
                if item.id == variant.runtime_id("libstdc++")
            )
    ...
    def test_variant_id_names_the_complete_runtime_set(self) -> None:
        lock = resolve_lock(self.spec)
        clang = next(
            item
            for item in lock.variants
            if item.family == "clang" and item.version == "22.1.8"
        )
        self.assertTrue(
            clang.id.endswith("-libstdcxx-gcc-13.4.0-libcxx"),
            clang.id,
        )
    ...
    def test_missing_and_mismatched_artifact_references_are_rejected(self) -> None:
        ...
        missing["variants"][0]["runtimes"][0][  # type: ignore[index]
            "runtime_id"
        ] = "runtime-missing"
        with self.assertRaisesRegex(ConfigurationError, "missing runtime reference"):
        ...
        first["runtimes"][0]["runtime_id"] = other_runtime["id"]
