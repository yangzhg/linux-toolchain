...
from linux_toolchain.build_tools import DEFAULT_CMAKE_VERSION
from linux_toolchain.cli import _compact_builder_heartbeat, main
...
from linux_toolchain.smoke import SmokeRequest, SmokeResult
...
class CliRoutingTest(unittest.TestCase):
    ...
    def test_compacts_managed_and_build_tools_heartbeats(self) -> None:
        messages = {
            "managed: builder: compiling compiler-kit; elapsed: 2s\ncompiling source": (
                "managed: builder: compiling; elapsed: 2s\ncompiling source"
            ),
            "build tools: building CMake and native tools; elapsed: 2s\n"
            "linking cmake": "build tools: building; elapsed: 2s\nlinking cmake",
        }
        for message, expected in messages.items():
            with self.subTest(message=message):
                self.assertEqual(_compact_builder_heartbeat(message), expected)
    ...
    def test_setup_routes_the_selected_toolchain(self) -> None:
        ...
        self.assertIsNone(options["libstdcxx"])
        self.assertEqual(options["cmake_version"], DEFAULT_CMAKE_VERSION)
        self.assertTrue(options["install"])

    def test_setup_routes_the_clang_libstdcxx_provider(self) -> None:
        with patch(
            "linux_toolchain.cli.setup_toolchain",
            return_value=Path("/opt/toolchain/bin/lxtc"),
        ) as producer:
            status, _, _ = self.invoke(
                [
                    "setup",
        ...
        self.assertEqual(status, 0)
        self.assertEqual(producer.call_args.args, ("clang@19",))
        self.assertEqual(producer.call_args.kwargs["libstdcxx"], "gcc@10")
    ...
    def test_artifact_bundle_defaults_to_every_integration(self) -> None:
        ...
        self.assertEqual(status, 0)
        options = creator.call_args.kwargs
        self.assertEqual(options["build_tools"], Path("/artifacts/build-tools"))
    ...
    def test_sdk_create_routes_render_build_and_export(self) -> None:
        ...
        self.assertTrue(callable(builder.call_args.kwargs["source_progress"]))
    ...
    def test_verify_routes_to_packaged_runner(self) -> None:
        result = SmokeResult(
            path=Path("/build/result.json"),
        ...
        with patch("linux_toolchain.cli.run_smoke", return_value=result) as runner:
            status, stdout, _ = self.invoke(
                ["verify", "--binding", "/binding", "--build-dir", "/build"]
            )

        self.assertEqual(status, 0)
        self.assertEqual(stdout.strip(), "/build/result.json")
        request = runner.call_args.args[0]
        self.assertIsInstance(request, SmokeRequest)
        self.assertEqual(request.binding, Path("/binding"))
    ...
    def test_llvm_runtime_import_routes_source_proof(self) -> None:
        ...
        with (
            patch(
                "linux_toolchain.managed.publication.load_managed_llvm_source_evidence",
