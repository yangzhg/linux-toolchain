...
REQUESTED_ARCH = $(strip $(ARCH))
NORMALIZED_ARCH = $(if $(filter amd64,$(REQUESTED_ARCH)),x86_64,$(if $(filter arm64,$(REQUESTED_ARCH)),aarch64,$(REQUESTED_ARCH)))
TARGET_ARCH = $(if $(REQUESTED_ARCH),$(NORMALIZED_ARCH),$(NATIVE_ARCH))
COMPILER_NAME = $(subst @,,$(strip $(COMPILER)))
GLIBC_NAME = $(subst .,,$(strip $(GLIBC)))
CLANG_COMPILER = $(filter clang@%,$(strip $(COMPILER)))
LIBSTDCXX_SUFFIX = $(if $(CLANG_COMPILER),-$(subst @,,$(strip $(LIBSTDCXX))))
INTEGRATION_SUFFIX = $(if $(filter-out shell,$(strip $(INTEGRATION))),-$(strip $(INTEGRATION)))
CMAKE_SUFFIX = $(if $(filter-out 3.31.12,$(strip $(CMAKE_VERSION))),-cmake$(subst .,,$(strip $(CMAKE_VERSION))))
TOOLCHAIN_BASE = $(COMPILER_NAME)-glibc$(GLIBC_NAME)-$(TARGET_ARCH)
TOOLCHAIN_VARIANT = $(TOOLCHAIN_BASE)$(LIBSTDCXX_SUFFIX)$(INTEGRATION_SUFFIX)$(CMAKE_SUFFIX)
...
SETUP_ARGUMENTS = \
	$(if $(REQUESTED_ARCH),--arch "$(TARGET_ARCH)") \
	$(if $(CLANG_COMPILER),--libstdcxx "$(LIBSTDCXX)") \
...
setup:
	@if [ -z "$(strip $(COMPILER))" ] || [ -z "$(strip $(GLIBC))" ] || \
	   [ -z "$(strip $(PREFIX))" ] || [ -z "$(strip $(WORK_DIR))" ]; then \
		echo 'usage: make setup COMPILER=gcc@12 GLIBC=2.19 [LIBSTDCXX=gcc@12 for Clang] [CMAKE_VERSION=3.31.12] [PREFIX=$$HOME/.local/lib/linux-toolchain/gcc12-glibc219-$(TARGET_ARCH)] [WORK_DIR=$$PWD/out/work/gcc12-glibc219-$(TARGET_ARCH)] [STORE_DIR=/shared/linux-toolchain/store] [UBUNTU_SNAPSHOT=YYYYMMDDTHHMMSSZ]' >&2; \
		exit 2; \
	fi
...
bundle:
	@if [ -z "$(strip $(COMPILER))" ] || [ -z "$(strip $(GLIBC))" ] || \
	   ...
	   [ -z "$(strip $(BUNDLE_OUTPUT))" ]; then \
		echo 'usage: make bundle COMPILER=gcc@12 GLIBC=2.19 [LIBSTDCXX=gcc@12 for Clang] [CMAKE_VERSION=3.31.12] [WORK_DIR=$$PWD/out/work/gcc12-glibc219-$(TARGET_ARCH)] [STORE_DIR=/shared/linux-toolchain/store] [BUNDLE_OUTPUT=out/linux-toolchain-gcc12-glibc219-$(TARGET_ARCH).run] [BUNDLE_OPTIONS=--force] [UBUNTU_SNAPSHOT=YYYYMMDDTHHMMSSZ]' >&2; \
		exit 2; \
	fi
	@$(MAKE) --no-print-directory bootstrap
