...
def normalize_architecture(value: str) -> str:
    """Normalize the architecture aliases accepted by producer workflows."""
...
class SdkSpec:
    ...
    def load(cls, path: Path) -> "SdkSpec":
        return cls.from_dict(read_json_object(path, "SDK spec"))
    ...
    def from_dict(cls, data: object) -> "SdkSpec":
        ...
        target_data = object_value(
            data["target"], required=target_keys, context="target"
        )
        builder_data = object_value(
            data["builder"], required=builder_keys, context="builder"
        )
