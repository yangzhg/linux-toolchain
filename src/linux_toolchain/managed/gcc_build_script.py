...
readonly GCC_RELEASE_LDFLAGS="-Wl,--strip-debug"
readonly LIBGCC_DEBUG_FLAGS="-g0"
readonly GCC_CONFIG_ID="format=1 source=gcc-$VERSION target=$TARGET backend=$BACKEND_TARGET-$BACKEND_VERSION libquadmath={quadmath_mode} release_flags=$GCC_RELEASE_FLAGS release_ldflags=$GCC_RELEASE_LDFLAGS libgcc_debug_flags=$LIBGCC_DEBUG_FLAGS"
...
build_libgcc() {{
  make -C "$BUILD_ROOT/gcc" configure-target-libgcc
  make -C "$BUILD_ROOT/gcc/$TARGET/libgcc" -j"$JOBS" \
    LIBGCC2_DEBUG_CFLAGS="$LIBGCC_DEBUG_FLAGS" all
}}
{publish}
"""
    )
