...
def _compiler_kit_locations(
    ...
    linkers = {
        "default": f"compiler/bin/{triplet}-ld",
        "bfd": f"compiler/bin/{triplet}-ld.bfd",
        "gold": f"compiler/bin/{triplet}-ld.gold",
        "mold": f"compiler/bin/{triplet}-ld.mold",
    }
    if selection.family == "clang":
        linkers["lld"] = "compiler/bin/ld.lld"
    for context, relative in (
        ("C driver", cc),
        ("C++ driver", cxx),
        *((f"target tool {name}", relative) for name, relative in tools.items()),
        *((f"linker {name}", relative) for name, relative in linkers.items()),
    ):
...
def finalize_artifact(
    ...
    *,
    ...
    validate_artifact_symlinks(payload, context="managed artifact")
