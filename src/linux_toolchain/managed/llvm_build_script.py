...
install -m 0755 -T "$BUILD_ROOT/llvm/bin/ld.lld" \
  "$ARTIFACTS/compiler/bin/ld.lld"
...
"$ARTIFACTS/compiler/bin/clang++" --version | \
  grep -Eq "^clang version $VERSION([[:space:]-]|$)"
"$ARTIFACTS/compiler/bin/ld.lld" --version | \
  grep -Eq "^LLD $VERSION([[:space:](]|$)"
...
if test -z "$(find "{runtime_artifacts}/runtime" \
    -type f -name 'libc++experimental.a' -print -quit)"; then
  echo "managed libc++ runtime is missing libc++experimental.a" >&2
  exit 1
fi
"""
...
if test "$PRESERVE_PRIMARY" = 0; then
  cmake --build "$BUILD_ROOT/llvm" --parallel "$JOBS" \
    --target clang clang-resource-headers lld
fi
"""
        if paired_runtime
        else r"""
...
readonly LLVM_RELEASE_LDFLAGS="-static-libstdc++ -static-libgcc -Wl,--strip-debug"
readonly LLVM_CONFIG_ID="format=1 sha512=$SOURCE_SHA512 target=$TARGET backend=$BACKEND_TARGET-$BACKEND_VERSION projects=clang,lld linkage=both experimental=on runtime_dir=os-arch release_ldflags=$LLVM_RELEASE_LDFLAGS"
