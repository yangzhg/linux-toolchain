...
from linux_toolchain.schema import object_value
...
from linux_toolchain.source_archive import (
    archive_matches as _archive_matches,
)
from linux_toolchain.source_archive import (
    download_pinned_archive as _download_archive,
)
from linux_toolchain.source_archive import (
    publish_archive_file as _publish_archive_file,
)
...
_CROSSTOOL_NG_1_28_PATCH = (
    "patches/crosstool-ng/1.28.0/0001-linux-toolchain-support.patch"
)
MOLD_VERSION = "2.41.0"
_MOLD_ARCHIVE_FILENAME = f"v{MOLD_VERSION}.tar.gz"
MOLD_SOURCE_URL = (
    f"https://github.com/rui314/mold/archive/refs/tags/{_MOLD_ARCHIVE_FILENAME}"
)
MOLD_SHA256 = "0a61abac85d818437b425df856822e9d6e9982baeae5a93bcb02fe6c0060c61a"
_TARGET_TOOLS = (
    "ar",
    "as",
    "ld",
...
class _BuilderContract:
    ...
    def default_image_name(self) -> str:
        platform = self.platform.removeprefix("linux/")
        return (
            "linux-toolchain-builder:"
            f"{self.release.version}-{platform}-{self.sha256[:16]}"
        )

    def workspace_provenance(self) -> dict[str, object]:
        return {
            "builder_dockerfile_sha256": self.dockerfile_sha256,
            "builder_contract_sha256": self.sha256,
            "builder_patch": {
                "path": self.patch.relative_path,
                "sha256": self.patch.sha256,
            },
            "builder_base_image": self.release.builder_base_image,
            "builder_platform": self.platform,
            "builder_apt_snapshot": self.apt_snapshot,
        }


@dataclass(frozen=True)
class _ResolvedBuilderImage:
    name: str
    image: BuilderImage
...
def _gnu_archive(filename: str, path: str, sha256: str) -> PinnedArchive:
    return pinned_gnu_archive(
        filename=filename,
        path=path,
        sha256=sha256,
        preferred_base_url=os.environ.get(_GNU_MIRROR_ENV),
    )


def _component_archives(spec: SdkSpec) -> dict[str, PinnedArchive]:
    result: dict[str, PinnedArchive] = {}
    for component, version in _selected_component_versions(spec).items():
        ...
        if component == "linux":
            ...
            archive = _gnu_archive(
                filename,
                f"gcc/gcc-{version}/{filename}",
                COMPONENT_SHA256[(component, version)],
            )
        else:
            archive = _gnu_archive(
                filename,
        ...
        result[component] = archive
    return result


def _support_archives(spec: SdkSpec) -> dict[str, PinnedArchive]:
    ...
    return {
        ...
        "gmp": _gnu_archive(
            ...
            path="gmp/gmp-6.3.0.tar.xz",
            sha256="a3c2b80201b89e68616f4ad30bc66aee4927c3ce50e33929ca819d5c43538898",
        ),
        "mpfr": _gnu_archive(
            filename="mpfr-4.2.2.tar.xz",
            path="mpfr/mpfr-4.2.2.tar.xz",
        ...
        "mpc": _gnu_archive(
            ...
            path="mpc/mpc-1.3.1.tar.gz",
            sha256="ab642492f5cf882b74aa0cb730cd410a81edcdbec895183ce930e706c1c759b8",
        ),
        "ncurses": _gnu_archive(
            filename="ncurses-6.5.tar.gz",
            path="ncurses/ncurses-6.5.tar.gz",
            sha256="136d91bc269a9a5785e5f9e980bc76ab57428f604ce3e5a5a90cebc767971cc6",
        ),
        "libiconv": _gnu_archive(
            filename="libiconv-1.18.tar.gz",
            path="libiconv/libiconv-1.18.tar.gz",
            sha256="3b08f5f4f9b4eb82f151a7040bfd6fe6c6fb922efe4b1659c66ea933276965e8",
        ),
        "gettext": _gnu_archive(
            filename="gettext-0.26.tar.xz",
            path="gettext/gettext-0.26.tar.xz",
            sha256="d1fb86e260cfe7da6031f94d2e44c0da55903dbae0a2fa0fae78c91ae1b56f00",
        ),
        "mold": PinnedArchive(
            filename=_MOLD_ARCHIVE_FILENAME,
            source_url=MOLD_SOURCE_URL,
            sha256=MOLD_SHA256,
        ),
    }


def _source_archives(spec: SdkSpec) -> dict[str, PinnedArchive]:
    return {**_component_archives(spec), **_support_archives(spec)}


def _read_packaged_patch(relative_path: str) -> bytes:
    resource = files("linux_toolchain.resources").joinpath(*Path(relative_path).parts)
    try:
        return resource.read_bytes()
    except OSError as error:
        raise ConfigurationError(
            f"cannot read packaged crosstool-NG patch {relative_path}: {error}"
        ) from error


def packaged_crosstool_ng_patch(version: str) -> PackagedPatch:
    if version != "1.28.0":
        raise ConfigurationError(
            f"no packaged backend patch for crosstool-NG {version}"
        )
    return PackagedPatch(
        relative_path=_CROSSTOOL_NG_1_28_PATCH,
        content=_read_packaged_patch(_CROSSTOOL_NG_1_28_PATCH),
    )


def write_crosstool_ng_patch_context(version: str, context: Path) -> Path:
    patch = packaged_crosstool_ng_patch(version)
    destination = context / Path(patch.relative_path).name
    if destination.is_symlink():
        raise ConfigurationError(
            f"builder patch context cannot be a symlink: {destination}"
        )
    try:
        destination.write_bytes(patch.content)
        destination.chmod(0o644)
    except OSError as error:
        raise ExternalToolError(
            f"cannot write crosstool-NG builder patch {destination}: {error}"
        ) from error
    return destination
...
def _builder_build_args(
    ...
    return builder_image_build_args(
        ...
        crosstool_ng_patch=Path(patch.relative_path).name,
        crosstool_ng_patch_sha256=patch.sha256,
    )
...
def _builder_contract(
    ...
    *,
    ...
    release = CROSSTOOL_NG_RELEASES[spec.builder.version]
    patch = packaged_crosstool_ng_patch(release.version)
    platform = _builder_platform(spec, release)
    snapshot = ubuntu_builder_snapshot() if apt_snapshot is None else apt_snapshot
    digest = builder_image_contract_digest(
...
def sdk_producer_identity(spec: SdkSpec) -> dict[str, object]:
    ...
    spec.validate()
    contract = _builder_contract(spec, packaged_builder_dockerfile_sha256())
    builder = {
    ...
    return {
        ...
        "builder": builder,
        "backend_source": {
            "sha256": contract.release.sha256,
        },
        "component_sources": [
            {
            ...
            for archive in _source_archives(spec).values()
        ],
        "builder_contract": {
            "dockerfile_sha256": contract.dockerfile_sha256,
            "base_image": contract.release.builder_base_image,
            "platform": contract.platform,
            "apt_snapshot": contract.apt_snapshot,
            "sha256": contract.sha256,
        },
    }


def validate_sdk_producer_identity(
    value: object,
    *,
    context: str,
) -> tuple[dict[str, object], SdkSpec]:
    """Validate and reconstruct the SDK selected by a producer identity."""
    ...
    spec = SdkSpec.from_dict(
        {
            ...
            "name": "producer-identity-sdk",
            "target": data["target"],
            "builder": data["builder"],
        }
    )
    expected = sdk_producer_identity(spec)
    if data != expected:
        raise ConfigurationError(f"{context} does not match pinned SDK inputs")
...
def _resolve_workspace_builder_image(
    contract: _BuilderContract,
    workspace: Path,
    *,
    dockerfile: Path,
    image_name: str | None,
...
) -> _ResolvedBuilderImage:
    selected_name = image_name or contract.default_image_name
    archive_filename = f"crosstool-ng-{contract.release.version}.tar.xz"
    _emit(
        progress,
        f"{progress_context}: acquiring crosstool-NG {archive_filename}",
    )
    archive = _backend_archive(
        contract.release,
        workspace,
        source_cache=source_cache,
    ...
    if not _archive_matches(context_archive, contract.release.sha256):
        _publish_archive_file(archive, context_archive)
    write_crosstool_ng_patch_context(contract.release.version, build_context)
    build_args = _builder_build_args(
        contract.release,
        context_archive,
        contract.apt_snapshot,
        contract.patch,
    )

    def build_image() -> None:
        _emit(progress, f"{progress_context}: preparing pinned Docker builder image")
        run_streaming(
            docker_build_command(
    ...
    if resolution.cache_hit:
        _emit(progress, f"{progress_context}: using cached Docker builder image")
    return _ResolvedBuilderImage(name=selected_name, image=resolution.image)
...
def _record_builder_provenance(
    ...
    *,
    contract: _BuilderContract,
    resolved: _ResolvedBuilderImage,
) -> None:
    if resolved.image.platform != contract.platform:
        raise ExternalToolError(
            f"builder image platform is {resolved.image.platform!r}; expected "
    ...
    try:
        ...
        data.update(contract.workspace_provenance())
        data["builder_image"] = {
            **resolved.image.to_dict(),
            "name": resolved.name,
...
def acquire_workspace_builder_image(
    spec: SdkSpec,
    workspace: Path,
    *,
    jobs: int = 1,
    ...
    """Resolve the pinned builder image recorded by one completed SDK workspace."""

    workspace_spec = load_workspace(workspace)
    if workspace_spec != spec:
        raise ConfigurationError(
            "SDK workspace does not match the requested builder-image selection"
        )
    if not workspace_satisfies_build_goal(spec, workspace, FULL_BUILD_GOAL):
        raise ConfigurationError(
            "SDK workspace has no completed compiler backend for host-tool builds"
        )
    dockerfile = packaged_builder_dockerfile().resolve()
    dockerfile_sha256 = _validate_builder_dockerfile(dockerfile)
    contract = _builder_contract(spec, dockerfile_sha256)
    try:
        ...
        raise ConfigurationError(
            f"cannot verify workspace builder image: {error}"
        ) from error
    workspace_data = _require_workspace_envelope(
        workspace_data,
        context="workspace manifest",
    )
    recorded_image = workspace_data.get("builder_image")
    recorded_name = (
        recorded_image.get("name") if isinstance(recorded_image, dict) else None
    )
    image_name = (
        recorded_name if isinstance(recorded_name, str) and recorded_name else None
    )
    resolved = _resolve_workspace_builder_image(
        contract,
        workspace,
        dockerfile=dockerfile,
...
def _publish_mold_target_tool(spec: SdkSpec, workspace: Path) -> None:
    """Publish crosstool-NG's Mold output in the private target-tool tree."""

    target_root = workspace / "toolchain" / spec.target.triplet
    source = target_root / "bin" / "ld.mold"
    destination = workspace / "toolchain" / "bin" / f"{spec.target.triplet}-ld.mold"
    try:
        resolved = source.resolve(strict=True)
        resolved.relative_to(target_root.resolve(strict=True))
    except (OSError, RuntimeError, ValueError) as error:
        raise ExternalToolError(
            f"crosstool-NG did not produce a contained Mold linker: {source}"
        ) from error
    if not resolved.is_file() or not os.access(resolved, os.X_OK):
        raise ExternalToolError(
            f"crosstool-NG Mold linker is not executable: {resolved}"
        )
    ...
    try:
        ...
        raise ExternalToolError(
            f"cannot publish crosstool-NG Mold linker {destination}: {error}"
        ) from error
...
def workspace_satisfies_build_goal(
    ...
    *,
    ...
    try:
        ...
        contract = _builder_contract(spec, requested_dockerfile_sha256)
        requested_contract_sha256 = (
            contract.sha256
            if builder_contract_sha256 is None
            else builder_contract_sha256
        )
        expected_provenance = contract.workspace_provenance()
        expected_provenance["builder_contract_sha256"] = requested_contract_sha256
        defconfig_sha256 = file_sha256(defconfig_path)
        if (
            any(
                workspace_value.get(key) != value
...
def build_with_docker(
    ...
    *,
    ...
    expected_builder = contract.workspace_provenance()
    ...
    if workspace_satisfies_build_goal(
        ...
        selected_goal,
        dockerfile_sha256=dockerfile_sha256,
        builder_contract_sha256=contract.sha256,
    ):
        _emit(progress, f"sdk: reusing completed crosstool-NG {selected_goal} build")
        return
    host = _preflight_builder_host(contract.platform)
    identity = _write_container_identity_files(workspace, host)
    for component, pinned_archive in _source_archives(spec).items():
        _emit(progress, f"sdk: acquiring {component} {pinned_archive.filename}")
        _download_archive(
            pinned_archive,
            workspace,
            description=f"{component} source archive",
            source_cache=source_cache,
            progress=source_progress,
        )
    resolved_image = _resolve_workspace_builder_image(
        contract,
        workspace,
        dockerfile=dockerfile,
        image_name=image,
    ...
    docker = _docker_args(
        workspace.resolve(),
        resolved_image.image.image_id,
        builder_platform=contract.platform,
        identity=identity,
    )
    ...
    _publish_mold_target_tool(spec, workspace)
...
def export_sdk(spec: SdkSpec, workspace: Path) -> Path:
    ...
    builder_contract_sha256 = workspace_data.get("builder_contract_sha256")
    builder_patch = workspace_data.get("builder_patch")
    ...
    if (
        ...
        or not isinstance(builder_contract_sha256, str)
        or not isinstance(builder_patch, dict)
    ...
    packaged_dockerfile_sha256 = packaged_builder_dockerfile_sha256()
    contract = _builder_contract(spec, packaged_dockerfile_sha256)
    if dockerfile_sha256 != packaged_dockerfile_sha256:
        raise ConfigurationError(
            "workspace builder Dockerfile base-image provenance cannot be "
            "verified; rebuild with the packaged Dockerfile"
        )
    if builder_base_image != contract.release.builder_base_image:
        raise ConfigurationError(
            "workspace builder base image does not match the selected backend "
            f"release: expected {contract.release.builder_base_image!r}, "
            f"recorded {builder_base_image!r}"
        )
    if builder_apt_snapshot != contract.apt_snapshot:
        raise ConfigurationError(
            "workspace builder apt snapshot does not match the current "
            "package-source selection"
        )
    if builder_contract_sha256 != contract.sha256 or builder_patch != {
        "path": contract.patch.relative_path,
        "sha256": contract.patch.sha256,
    }:
        raise ConfigurationError(
            "workspace builder inputs do not match the packaged crosstool-NG backend"
        )
    if builder_platform != contract.platform or image_platform != builder_platform:
        raise ConfigurationError(
            "workspace builder platform does not match the selected backend "
            f"release: expected {contract.platform!r}, recorded "
    ...
    extract_component_licenses(
        workspace / "downloads" / _MOLD_ARCHIVE_FILENAME,
        temporary,
        "mold",
    )
    ...
    sdk_manifest = {
        ...
        "build_environment": {
            "dockerfile_sha256": dockerfile_sha256,
            "contract_sha256": builder_contract_sha256,
        ...
        "sources": {
            "crosstool-ng": {
                ...
                "url": contract.release.source_url,
                "sha256": contract.release.sha256,
                "patch": dict(builder_patch),
            },
            **{
                component: {
                    "version": version,
