...
from linux_toolchain.smoke import SmokeRequest, run_smoke
...
def _compact_builder_heartbeat(message: str) -> str | None:
    for marker in ("builder: compiling ", "build tools: building "):
        prefix, separator, detail = message.partition(marker)
        if not separator:
            continue
        heartbeat, _, output = detail.partition("\n")
        _, separator, elapsed = heartbeat.partition("; elapsed: ")
        if not separator:
            return None
        live_message = (
            f"{prefix}{marker.rstrip()}; elapsed: {elapsed.partition(';')[0]}"
        )
            ...
            [
                {
                    ...
                    "runtimes": ",".join(entry.cxx_runtimes),
                    "target": entry.target.arch,
                    "glibc": entry.target.glibc_floor,
                }
