...
_RUNTIME_LIBRARY_NAME = re.compile(
    r"^(?:"
    r"libc\+\+experimental\.a|"
    r"(?:libc\+\+abi|libc\+\+|libunwind)(?:\.a|\.so(?:\.[0-9]+)*)"
    r")$"
)
...
def _inspect_static_libraries(
    *,
    ...
    for relative in paths:
        ...
        if path.name == LLVM_EXPERIMENTAL_STATIC_LIBRARY:
            component = "libc++experimental"
        elif component is None or path.name != f"{component}.a":
...
def import_llvm_runtime(
    ...
    *,
    ...
    try:
        expected = materialize_llvm_runtime(
            source,
            llvm_version,
            glibc_floor,
            arch,
