...
from linux_toolchain.models import classify_linux_glibc_target, normalize_architecture
...
def _locations(value: object, family: str) -> Mapping[str, object]:
    context = "compiler kit manifest.locations"
    data = _object(
        value,
        required={"cc", "cxx", "target_tools", "linkers"},
        context=context,
    )
    tool_data = _object(
        data["target_tools"],
    ...
    required_linkers = (
        {"default", "bfd", "gold", "mold", "lld"}
        if family == "clang"
        else {"default", "bfd", "gold", "mold"}
    )
    linker_data = _object(
        data["linkers"],
        required=required_linkers,
        context=f"{context}.linkers",
    )
    linkers = MappingProxyType(
        {
            name: _relative_payload_path(linker_data[name], f"{context}.linkers.{name}")
            for name in sorted(required_linkers)
        }
    )
    return MappingProxyType(
        {
...
class CompilerKitManifest:
    ...
    def from_dict(cls, value: object) -> "CompilerKitManifest":
        ...
        return cls(
            ...
            locations=_locations(data["locations"], family),
        )

    def to_dict(self) -> dict[str, object]:
        tools = self.locations["target_tools"]
        linkers = self.locations["linkers"]
        assert isinstance(tools, Mapping)
        assert isinstance(linkers, Mapping)
        return {
            "schema": self.schema,
            "format": self.format,
            ...
            "locations": {
                ...
                "linkers": {name: linkers[name] for name in sorted(linkers)},
            },
        }
...
class CompilerKit:
    ...
    linkers: Mapping[str, CompilerKitExecutable]
...
def load_compiler_kit(
    ...
    *,
    ...
    manifest = CompilerKitManifest.from_dict(
        read_json_object(manifest_path, "compiler kit manifest")
    )
    if check_host:
    ...
    linker_locations = manifest.locations["linkers"]
    assert isinstance(linker_locations, Mapping)
    linkers = MappingProxyType(
        {
            name: _load_executable(
                root,
                compiler_root,
                relative,
                f"compiler kit linker {name}",
            )
            for name, relative in linker_locations.items()
        }
    )
    return CompilerKit(
        root=root,
