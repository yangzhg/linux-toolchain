...
def packaged_builder_dockerfile() -> Path:
    return Path(
        str(files("linux_toolchain.resources").joinpath(BUILDER_DOCKERFILE_NAME))
    )


def packaged_builder_dockerfile_bytes() -> bytes:
    try:
        return packaged_builder_dockerfile().read_bytes()
    except OSError as error:
        raise ConfigurationError(
            f"cannot read packaged builder Dockerfile: {error}"
        ) from error


def packaged_builder_dockerfile_sha256() -> str:
    return hashlib.sha256(packaged_builder_dockerfile_bytes()).hexdigest()
...
def container_run_command(
    *,
    ...
    """Build the common isolated producer-container invocation."""
    ...
    identity_mounts = (
        ContainerMount(identity.passwd, "/etc/passwd", read_only=True),
        ContainerMount(identity.group, "/etc/group", read_only=True),
    )
    for mount in (*mounts, *identity_mounts):
...
def docker_build_command(
    *,
    ...
    if not isinstance(jobs, int) or isinstance(jobs, bool) or jobs < 1:
        raise ConfigurationError("builder image jobs must be a positive integer")
    command = [
        "docker",
        "build",
        "--platform",
    ...
    command.extend(("--build-arg", f"LINUX_TOOLCHAIN_JOBS={jobs}"))
...
def builder_image_contract_digest(
    *,
    ...
    """Identify a builder image from content inputs, excluding execution options."""
...
def builder_image_build_args(
    *,
    base_image: str,
    apt_snapshot: str,
    crosstool_ng_version: str,
    crosstool_ng_sha256: str,
    crosstool_ng_archive: str,
    crosstool_ng_patch: str,
    crosstool_ng_patch_sha256: str,
) -> dict[str, str]:
    """Return the content-defining build arguments shared by producer workflows."""

    return {
        "BASE_IMAGE": base_image,
        "UBUNTU_SNAPSHOT": apt_snapshot,
        "CROSSTOOL_NG_VERSION": crosstool_ng_version,
        "CROSSTOOL_NG_SHA256": crosstool_ng_sha256,
        "CROSSTOOL_NG_ARCHIVE": crosstool_ng_archive,
        "CROSSTOOL_NG_PATCH": crosstool_ng_patch,
        "CROSSTOOL_NG_PATCH_SHA256": crosstool_ng_patch_sha256,
    }
