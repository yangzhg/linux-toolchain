...
def _shell_literal(value: str) -> str:
    """Quote values unconditionally so later bundle relocation remains safe."""
...
def render_shell(
    ...
    *,
    ...
    assignments = "\n".join(
        f"export {name}={_shell_literal(value)}" for name, value in values.items()
    )
    quoted_bin = _shell_literal(str(context.binding_root / "bin"))
