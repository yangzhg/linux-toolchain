...
from linux_toolchain.models import SUPPORTED_ARCHITECTURES, normalize_architecture
...
def default_dockerfile() -> Path:
    return packaged_builder_dockerfile()
...
def _add_verify_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        default="cmake",
        help="build-system integration to exercise",
    )
    parser.add_argument(
        "--build-type",
        choices=("Debug", "Release", "RelWithDebInfo", "MinSizeRel"),
        default="Release",
        help="consumer build configuration",
    )
    ...
    parser.add_argument(
        ...
        help="Make backend used by every verification integration",
    )
    ...
    *,
    ...
    if conan_libcxx:
        parser.add_argument(
            ...
            help=(
                "Conan compiler.libcxx setting; inferred from the runtime selection "
...
def build_parser() -> argparse.ArgumentParser:
    ...
    setup.add_argument(
        ...
        help="primary consumer verification integration; all adapters are included",
    )
    setup.add_argument(
        "--libstdcxx",
        help="libstdc++ provider bundled with Clang; defaults to gcc@12",
    )
    setup.add_argument(
        "--cmake-version",
        default=DEFAULT_CMAKE_VERSION,
        help="managed CMake, CTest and CPack version",
    )
    setup.add_argument(
        "--prefix",
        type=Path,
    ...
    setup_advanced.add_argument(
        "--runner",
        help="optional command prefix for consumer verification",
    )
    setup_advanced.add_argument(
    ...
    setup_conan.add_argument(
        "--conan-cppstd",
        help="compiler.cppstd for the generated Conan host profile",
    )
    setup_conan.add_argument(
        "--conan-build-type",
        choices=("Debug", "Release", "RelWithDebInfo", "MinSizeRel"),
        help=(
            "build type for the generated Conan host profile and, when selected, "
            "the producer Conan verification"
        ),
    )
    setup_conan.add_argument(
        "--conan-build-profile",
        help=(
            "native build profile for producer Conan verification; requires "
            "--integration conan"
        ),
    )
    setup.add_argument(
    ...
    managed_binding = bind_commands.add_parser(
        "managed", help="bind a managed compiler kit and target runtime set"
    )
    ...
    bundle_artifacts = bundle_commands.add_parser(
        ...
        description=(
            "Advanced release interface. Validate and package one SDK, build-tools "
            "artifact, Compiler Kit, runtime set, managed lock and binding template."
        ),
    )
    bundle_artifacts.add_argument("--sdk", required=True, type=Path)
    bundle_artifacts.add_argument("--build-tools", required=True, type=Path)
    ...
    managed_assemble = managed_commands.add_parser(
        ...
        description=(
            ...
            "resume validated artifacts, publish the runtime set and create a binding. "
    ...
    verify = commands.add_parser(
        ...
        description=(
            "Verify a pinned-runtime binding with the packaged C++/ASM "
