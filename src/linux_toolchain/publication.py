...
def file_lock(
    ...
    *,
    ...
    """Hold a symlink-safe advisory lock for one filesystem operation."""
    ...
    try:
        ...
        raise ConfigurationError(
            f"cannot open {context} lock {path}: {error}"
        ) from error
    with os.fdopen(descriptor, "r+", encoding="ascii") as stream:
        try:
            ...
            raise ConfigurationError(
                f"cannot acquire {context} lock {path}: {error}"
            ) from error
        yield
...
def replace_directory(
    ...
    destination: Path,
    *,
    validate: Callable[[Path], _Validated] | None = None,
) -> _Validated | None:
    ...
    validated: _Validated | None = None
    try:
