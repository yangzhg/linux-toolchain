        ...
        with tarfile.open(archive, mode="r:*") as source:
            for member in sorted(source.getmembers(), key=lambda item: item.name):
                ...
                if relative is None or member.isdir() or not _license_path(relative):
                    continue
                if not member.isreg():
                    if relative.as_posix() not in requirements:
                        continue
                    raise ExternalToolError(
...
def managed_required_license_paths(
    ...
    *,
    ...
    return (*required, "binutils/COPYING", "mold/LICENSE") if compiler_kit else required
...
def validate_license_manifest(root: Path, *, context: str) -> dict[str, object]:
    ...
    value = read_json_object(path, f"{context} license manifest")
