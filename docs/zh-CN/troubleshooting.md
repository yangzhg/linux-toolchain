...
编译器 CRT 和头文件。managed Clang Bundle 还携带 libc++；需要让 C++ 依赖闭包避免
GCC runtime SONAME 时，可用 Clang 原生 runtime 选项选择 libc++。检查范围应覆盖完整
部署目录，而不只是主共享库。
...
检查验证构建目录中的 `audit-report.json`、`loader-closure.txt` 和
