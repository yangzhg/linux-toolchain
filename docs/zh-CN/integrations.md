...
| 使用方 | 选择 | 生成内容 | 验证方式 |
| --- | --- | --- | --- |
| CMake | `--integration cmake` | 独立 toolchain file | 专用验证 |
| POSIX shell / Make | `--integration shell` | compiler、目标工具和 pkg-config 环境 | Make 验证 |
| Conan 2 | `--integration conan` | host profile 与 CMake 组合片段 | 专用验证 |
...
高层 setup 在 x86-64 或 AArch64 producer 上原生运行，target 默认取 producer 架构，
因此普通 native 场景无需指定 `--arch`。发布者还可设置 `--host-glibc-floor`、`--jobs`
和 `--runner`。managed Clang 同时包含匹配 LLVM libc++ 和 GCC libstdc++/libgcc runtime；
`--libstdcxx gcc@VERSION` 选择 GCC provider，默认是 `gcc@12`。高层 setup 始终生成
CMake、shell 和 Conan adapter；`--integration` 选择其中哪一个接受生成端验证，
选择 Conan 时还会启用该生成端验证的 `--conan-*` 选项。
...
`lxtc shell` 可进入相同环境下的交互式子 shell。banner 会显示已安装的编译器、
target、libc 下限和实际 C++ runtime。它读取用户正常的 shell 启动文件，随后重新
应用工具链环境，只把所选 runtime 的共享库目录放到子 shell 的 `LD_LIBRARY_PATH`
前面，并只替换常规提示符中第一个 `:` 之前的前缀。此时直接执行程序仍使用主机
loader 和 libc；退出后父 shell 保持不变。双 runtime Clang shell 可替换成另一个
快照：
...
动态链接的 target 可执行文件需要改用 SDK loader、所选 C++ runtime 和 SDK libc 时，
使用 `lxtc run EXECUTABLE [ARG ...]`。执行前，launcher 会检查解析结果没有回退到
未选择的 managed runtime 或系统库目录。
...
profile。两个 context 都使用安装后的托管工具链，并在各自的 Conan build environment
中把当前 C++ runtime 目录放在搜索路径前面。这样，由 Conan 作为 build requirement
构建的原生工具随后被 host package 调用时仍能找到同一套 runtime，同时不会给所有
经 `lxtc` 启动的命令全局导出 `LD_LIBRARY_PATH`；上面显式进入的 `lxtc shell`
是例外。不同的既有 settings 或 profile 会被拒绝。显式传
`--profile:host`/`--profile:build` 时，可读取实际路径
`LINUX_TOOLCHAIN_CONAN_HOST_PROFILE` 和
`LINUX_TOOLCHAIN_CONAN_BUILD_PROFILE`。
双 runtime Clang Bundle 可通过 `lxtc runtime set` 持久切换两个 context，也可通过
`lxtc --runtime` 只覆盖一条命令；`lxtc conan-init` 只重新创建 Conan 静态配置。
生成的 `lxtc.info` 包含安装身份及持久选择。
...
所有 managed Bundle 都发布 BFD、Gold 和 Mold；Clang Bundle 还发布 LLD，默认仍是
BFD。Clang 和 GCC 12 及以上版本通过 compiler 原生的 `-fuse-ld` 选择；GCC 10 和 11
使用 `cc-mold` 或 `c++-mold` 选择 Mold。managed Clang 默认使用 libstdc++/libgcc，
也接受原生的 `-stdlib=libc++`、`--rtlib=compiler-rt` 和
`--unwindlib=libunwind`。binding 不解析或改写这些选项及 `--target`。
...
默认值。托管 binding 的 variant 记录可用 C++ runtime；Conan libcxx 必须选择其中
一种，未指定时使用 variant 默认值。
...
双 runtime Clang Bundle 还会创建 `lxtc-libstdcxx`、`lxtc-libcxx` 目标选择器以及
对应的原生 build profile：
...
持久选择或命令级选择会同时一致地作用于 Conan host 和默认原生 build context。
`conan-init` 会从已安装 binding 重新创建缺失的静态配置并刷新 `lxtc.info`，不运行
Conan，也不改写已有不同 profile。

`--conan-cppstd VALUE` 只覆盖目标 profile；`--conan-build-profile NAME_OR_PATH` 显式
替换生成的 build context，是使用方决定自行管理该 context 时的逃生口，runtime 选择
不会重映射它。这一 Bundle 行为依赖高层 managed production 为原生模式，不会扩散到
通用底层 binding。
...
描述 target；优先使用 target 配置、编译/链接 probe 和显式 cache 答案。使用方验证和
最终 consumer test 要在有代表性的 target 机器上运行；静态 configure 成功不等于
target 认证通过。
