...
build 目录必须为空，或由之前的验证运行创建。

验证通过后，构建目录会保留命令日志、`audit-report.json`、`loader-closure.txt`、
`runtime-output.txt` 和 `result.json`。详细命令及证据输出只在失败时回放。结果中记录
构建类型、接入方式、策略下限，以及检查到的最高 GLIBC 版本。日常开发应为每个绑定运行
这个项目。正式发布仍需从已知来源构建真实项目及其依赖，递归检查部署目录，并在符合
最低内核、动态加载器、架构和 CPU 策略的环境中运行。
