...
control libstdc++, libgcc, compiler CRT and their headers. A managed Clang
bundle also carries libc++; select it with Clang's native runtime options when
the resulting C++ closure must avoid GCC runtime SONAMEs. Audit the final
deployment tree, not only the main shared object.
...
## The verification project compiles but does not run

Inspect `audit-report.json`, `loader-closure.txt` and `runtime-output.txt` in the
verification build directory. Cross targets need an explicit runner such as
...
bindings contain all three. If verification or a consumer reports a missing
