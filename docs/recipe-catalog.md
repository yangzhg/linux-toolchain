...
| Family | Available glibc entries | crosstool-NG | Builder platform | Linux headers | Compiler backend | binutils | Mold |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `crosstool-ng-1.28.0` | 2.17, 2.19, and 2.23 through 2.42 | 1.28.0 | `linux/amd64`, `linux/arm64` | 6.12.41 | GCC 9.5.0 | 2.43.1; AArch64 with glibc older than 2.26 uses 2.29.1 | 2.41.0 |
...
entries pin binutils 2.29.1 rather than 2.43.1. Use
...
The glibc 2.17 and 2.19 backend patch series include the upstream BZ #16381
fix. It makes an explicitly loaded PIE main program an executable in the
dynamic loader and preserves one-time constructor execution. Other catalog
entries do not receive this backport; the later catalog entries already contain
the upstream fix.
...
Catalog-generated specifications use the complete canonical targets
`x86_64-unknown-linux-gnu` and `aarch64-unknown-linux-gnu`. The `unknown`
component is the conventional vendor field; it does not identify a producer or
change the Linux glibc ABI.
...
is audited against the resolved host floor, and target binutils plus Mold are
required to be static so the builder-container libc cannot leak into the
Compiler Kit.
...
requires a real build, consumer verification, ELF audit, loader-closure check and
...
Linux, GCC, binutils, Mold, builder-platform, source and hash selections. The
resolver does not guess a backend for an unknown glibc release.
