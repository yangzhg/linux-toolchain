...
| --- | --- | --- | --- |
| CMake | `--integration cmake` | standalone toolchain file | dedicated verification |
| POSIX shell / Make | `--integration shell` | compiler, target-tool and pkg-config environment | dedicated Make verification |
| Conan 2 | `--integration conan` | host profile and CMake composition fragments | dedicated verification |
...
The high-level setup command runs natively on x86-64 or AArch64 and defaults
the target to the producer architecture, so `--arch` is unnecessary for the
normal native case. Release producers may also select `--host-glibc-floor`,
`--jobs`, and `--runner`. Managed Clang includes both matching LLVM libc++ and
a GCC libstdc++/libgcc runtime;
`--libstdcxx gcc@VERSION` selects the GCC provider and defaults to `gcc@12`.
High-level setup always renders CMake, shell, and Conan adapters.
`--integration` chooses which one receives producer verification; choosing
Conan also enables that verification path's `--conan-*` options.
...
Use `lxtc shell` for an interactive child shell in the same environment. Its
banner reports the installed compiler, target, libc floor, and effective C++
runtime. It loads the user's normal shell startup file, then reapplies the
toolchain environment, prepends only the selected runtime library directories
to the child shell's `LD_LIBRARY_PATH`, and changes only the conventional
prompt prefix before the first `:`. Direct child-shell execution uses the host
loader and libc. Exit returns to the unchanged parent shell. A dual-runtime
Clang shell can be replaced with another snapshot:

```bash
exec lxtc --runtime libc++ shell
```

Use `lxtc run EXECUTABLE [ARG ...]` for a dynamically linked target executable
that must instead use the SDK loader, selected C++ runtime, and SDK libc. The
launcher checks that resolution does not fall back to an unselected managed
runtime or system library directory before execution.
...
their Conan build environments prepend the selected C++ runtime directories.
This keeps native tools built as Conan build requirements runnable when a host
package later invokes them, without exporting `LD_LIBRARY_PATH` to every
command launched by `lxtc`. The interactive `lxtc shell` is the explicit
exception described above. A different existing settings or profile file is
rejected rather than overwritten. A consumer that passes `--profile:host` or
`--profile:build` explicitly can read the effective paths from
`LINUX_TOOLCHAIN_CONAN_HOST_PROFILE` and
`LINUX_TOOLCHAIN_CONAN_BUILD_PROFILE`.
For a dual-runtime Clang bundle, `lxtc runtime set` changes the persistent
launcher selection for both contexts and `lxtc --runtime` overrides both for
one command. `lxtc conan-init` only recreates static Conan configuration. The
generated `lxtc.info` contains the installed identity and persistent selection.
...
Managed bundles publish BFD, Gold and Mold; Clang bundles also publish LLD.
BFD remains the default. Clang and GCC 12 or newer use the compiler's native
`-fuse-ld` option. GCC 10 and 11 use `cc-mold` or `c++-mold` to select Mold.
Managed Clang defaults to libstdc++/libgcc and also accepts its native
`-stdlib=libc++`,
`--rtlib=compiler-rt`, and `--unwindlib=libunwind` options. The binding does not
parse or rewrite these options or `--target`.
...
records the available C++ runtimes. The Conan libcxx value must select one of
them and defaults to the variant's default.
...
Conan home without invoking Conan. A dual-runtime Clang bundle also creates
`lxtc-libstdcxx` and `lxtc-libcxx` target selectors plus matching native build
profiles:
...
lxtc conan-init
lxtc conan install . \
  --build=missing --output-folder=build/target
lxtc --runtime libstdc++ conan install . \
  --build=missing --output-folder=build/target
```

The persistent or command-scoped launcher selection applies coherently to the
Conan host and default native build contexts. `conan-init` recreates missing
static configuration from the installed binding and refreshes `lxtc.info`
without running Conan or changing a different existing profile.

`--conan-cppstd VALUE` overrides only the target profile.
`--conan-build-profile NAME_OR_PATH` replaces the generated build context
explicitly; it is an escape hatch for a consumer that chooses to own that
context, and runtime selection does not remap it. This bundle behavior relies
on high-level managed production being native and is intentionally not added
to generic low-level bindings.
...
Run verification and final consumer tests on a representative target machine. Static
