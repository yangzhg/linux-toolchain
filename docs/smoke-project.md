...
The verification project is the fast consumer check for a generated compiler binding.
...
The verification project is installed with the wheel. `BINDING` is the directory
...
## Direct CMake verification

Direct CMake is the default integration mode. It configures the verification project
...
## Make verification for the shell integration

`env/toolchain.env` is the build-system-neutral entry point for Make,
Autotools, hand-written Ninja graphs and other configure systems. The verification
...
## Verification outputs and build-directory reuse

The build directory must be empty or have been created by a previous
verification run.

A passing run leaves command logs, `audit-report.json`,
`loader-closure.txt`, `runtime-output.txt`, and `result.json` under the build
directory. Detailed command and evidence output is replayed only on failure.
The result records the consumer build type, integration mode, policy floor and
highest GLIBC version observed in the audited outputs.

Use the verification project on every binding used in normal development. A
release matrix must still build the real consumer and target dependencies from
known sources, recursively audit the deployment tree and run in an environment
representative of the minimum kernel, glibc loader, architecture and CPU
policy.
