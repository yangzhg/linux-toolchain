...
| --- | --- | --- |
| Compiler Kit host floor | every glibc-linked host ELF in the Compiler Kit and bundled build-tools artifact, including GCC or Clang, helper executables, libraries, binutils, CMake, Make and Ninja | oldest build host on which those programs may run |
...
binutils and Mold are statically linked and must expose no dynamic-loader,
...
The bundled build tools follow the same native architecture. CMake, CTest,
CPack, GNU Make, and Ninja are recursively audited against the Compiler Kit
host floor; their C++ dependencies are linked statically. ccache is the
architecture-matched static-musl executable, so it has no glibc version
requirement, interpreter, or `DT_NEEDED` closure. This does not make either
architecture interchangeable: every tool is still validated as x86-64 or
AArch64 ELF as selected.
...
A sysroot alone does not pin compiler-owned C++ inputs. A binding records one
or more of these runtime policies:
...
- Clang's libstdc++ input comes from an explicitly selected GCC runtime; the
  Clang and GCC version numbers are independent;
- Clang's libc++ input requires a managed Clang Compiler Kit and an LLVM
  runtime from the same exact LLVM release.
...
members are checked for the selected target. Managed libc++ builds also require
and publish `libc++experimental.a`; an external LLVM import preserves and
validates it when the selected prefix provides it. The binding never enables
experimental libc++ facilities or links that archive implicitly. The LLVM
runtime closure forbids `libstdc++.so.6` and `libgcc_s.so.1`.
...
`lxtc shell` is a development convenience: its child environment exposes only
the selected C++ runtime directories, while direct execution continues to use
the host loader and libc. `lxtc run` provides a stricter local check by using
the SDK loader and rejecting a closure that falls back to an unselected managed
runtime or system library directory. Neither command changes the output binary
or replaces deployment-time loader layout and recursive ELF auditing.
...
drivers and every target tool from the Compiler Kit manifest. Managed kits
provide BFD, Gold and Mold, while Clang kits also provide LLD.
...
machine-local because their paths describe one compiler installation. Clang
target options in command lines, configuration files and response files are
therefore interpreted by Clang rather than normalized by the binding. The
managed runtime layout supports equivalent vendor spellings for the same
architecture. An incompatible target fails in the driver or subsequent
validation. GCC uses its configured target-specific driver and does not support
...
Binding-owned linker search paths are added only when the driver invokes the
selected linker. An LLVM binding also supplies separate shared and static
libunwind linker entries; the static entry carries libunwind's pthread and
dynamic-loader dependencies without changing invocations that do not select
libunwind.
...
Static probes must have no dynamic interpreter or `DT_NEEDED` entries. Each
published linker is exercised through a driver. Clang and GCC 12 or newer use
native `-fuse-ld` selection. Because GCC 10 and 11 do not accept
`-fuse-ld=mold`, their bindings publish `cc-mold` and `c++-mold` drivers with a
fixed, binding-owned `-B` selector.

A managed Clang binding anchors native GCC discovery to the bundled GCC
runtime and exposes libc++ headers and compiler-rt through Clang's standard
installation layout. Clang then handles `-stdlib`, `--rtlib` and `--unwindlib`
itself. The Linux default remains libstdc++/libgcc; selecting
libc++/compiler-rt/libunwind uses the matching LLVM overlay. Producer probes
exercise both choices, repeated `-stdlib` arguments under
`-Werror=unused-command-line-argument`, an equivalent vendor target spelling,
and static links. LLVM-selected output must not depend on GCC runtime SONAMEs.
...
this boundary; standalone executable verification cannot replace the real
host-process check.
...
1. Archive the SDK, build-tools, compiler, runtime and binding manifests that
...
5. Run the small C++ shared-library/executable verification project for each binding.
...
Use the small verification project during routine development. Full consumer builds
